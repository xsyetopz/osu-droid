...
LEGACY = REPO / "third_party" / "osu-droid-legacy"
...
APP_ALIASES: "OrderedDict[str, tuple[str, str]]" = OrderedDict([
    ...
    ("Options_Unavailable", (LEGACY_KEY, "multiplayer_room_not_available_beatmap")),
    ...
    ("Options_LanguageValue", (LEGACY_KEY, "multiplayer_room_chat_system")),
    ("Options_LanguageSystemDefault", (LEGACY_KEY, "multiplayer_room_chat_system")),
    ...
    ("Options_OffsetCalibrationTitle", (LEGACY_KEY, "opt_offset_calibration_title")),
    ("Options_OffsetCalibrationSummary", (LEGACY_KEY, "opt_offset_calibration_summary")),
    ...
    ("Options_PreferNoVideoDownloadsTitle", (LEGACY_KEY, "opt_prefer_no_video_downloads_title")),
    ("Options_PreferNoVideoDownloadsSummary", (LEGACY_KEY, "opt_prefer_no_video_downloads_summary")),
    ...
    ("Options_HighPrecisionInputTitle", (LEGACY_KEY, "opt_highPrecisionInput_title")),
    ("Options_HighPrecisionInputSummary", (LEGACY_KEY, "opt_highPrecisionInput_summary")),
    ("Options_RemoveSliderLockTitle", (LEGACY_KEY, "opt_remove_sliderlock_spinnerlock_title")),
    ("Options_RemoveSliderLockSummary", (LEGACY_KEY, "opt_remove_sliderlock_spinnerlock_summary")),
    ...
    ("MainMenu_AboutClose", (LEGACY_KEY, "multiplayer_room_kicked_close")),
    ...
    ("MainMenu_ExitInstruction", (LANGUAGE_PACK_KEY, "dialog_exit_message")),
...
def resolve_alias(kind: str, source: str, legacy: OrderedDict[str, str], language_pack: OrderedDict[str, str]) -> str:
    if kind == LEGACY_KEY:
...
def generate_entries() -> OrderedDict[str, str]:
    legacy = read_values(LEGACY, english_only=True)
    ...
    for name, (kind, source) in APP_ALIASES.items():
        entries[name] = resolve_alias(kind, source, legacy, language_pack)

    for name, value in legacy.items():
        entries[f"Legacy_{name}"] = value

    for name, value in language_pack.items():
        entries[f"LegacyLanguagePack_{name}"] = value
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate English localization from legacy osu!droid sources.")
    ...
    if args.check:
        ...
        if current != generated:
            print("Strings.resx is out of date. Run scripts/dev/generate-legacy-localization.py", file=sys.stderr)
            return 1
        current_template = LOCALE_TEMPLATE.read_text()
        if current_template != generated:
            print("Strings.locale.resx.template is out of date. Run scripts/dev/generate-legacy-localization.py", file=sys.stderr)
            return 1
        return 0
