...
internal static class DifficultyCalculationUtils
{
    public static double BpmToMilliseconds(double bpm, int delimiter = 4) =>
        60000d / bpm / delimiter;

    public static double MillisecondsToBpm(double milliseconds, int delimiter = 4) =>
        60000d / (milliseconds * delimiter);

    public static double Logistic(double exponent, double maxValue = 1d) =>
        maxValue / (1 + System.Math.Exp(exponent));
