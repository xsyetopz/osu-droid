...
)
{
    ...
    public string GetAudioPath(string songsPath) =>
        Path.Combine(GetSetPath(songsPath), AudioFilename);

    public string? GetBackgroundPath(string songsPath) =>
        string.IsNullOrWhiteSpace(BackgroundFilename)
            ? null
            : Path.Combine(GetSetPath(songsPath), BackgroundFilename);

    public int EffectivePreviewTime =>
        Length <= 0
