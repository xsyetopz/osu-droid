...
public static class OsuDroidColors
{
    public static UiColor StarRating(float? starRating)
    {
        ...
        for (int i = 0; i < points.Length - 1; i++)
        {
            ...
            float amount = Math.Clamp(
                (rounded - current.Item1) / Math.Max(0.001f, next.Item1 - current.Item1),
                0f,
                1f
            );
            return new UiColor(
                (byte)
                    MathF.Round(current.Item2.Red + (next.Item2.Red - current.Item2.Red) * amount),
                (byte)
                    MathF.Round(
                        current.Item2.Green + (next.Item2.Green - current.Item2.Green) * amount
                    ),
                (byte)
                    MathF.Round(
                        current.Item2.Blue + (next.Item2.Blue - current.Item2.Blue) * amount
                    ),
                255
            );
        }
    ...
    public static UiColor Interpolate(
        float value,
        ReadOnlySpan<(float Domain, UiColor Color)> gradient
    )
    {
