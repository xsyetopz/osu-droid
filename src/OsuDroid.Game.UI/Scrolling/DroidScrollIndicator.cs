...
public static class DroidScrollIndicator
{
    ...
    )
    {
        ...
        float width =
            viewportBounds.Width * Math.Clamp(viewportBounds.Width / contentWidth, 0f, 1f);
        float x =
            viewportBounds.X
            + (viewportBounds.Width - width) * Math.Clamp(scrollOffset / maxScrollOffset, 0f, 1f);
    ...
    )
    {
        ...
        float height =
            viewportBounds.Height * Math.Clamp(viewportBounds.Height / contentHeight, 0f, 1f);
        float y =
            viewportBounds.Y
            + (viewportBounds.Height - height) * Math.Clamp(scrollOffset / maxScrollOffset, 0f, 1f);
        return UiElementFactory.Fill(
            id,
