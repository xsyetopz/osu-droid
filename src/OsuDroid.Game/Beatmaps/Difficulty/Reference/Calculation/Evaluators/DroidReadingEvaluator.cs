...
internal static class DroidReadingEvaluator
{
    ...
    private static readonly double DistanceInfluenceThreshold = DifficultyHitObject.NormalizedDiameter * 1.25;
    ...
    public static double EvaluateDifficultyOf(DroidDifficultyHitObject current, double clockRate, IEnumerable<Mod> mods)
    {
        ...
        double velocityFactor = System.Math.Max(1d, current.MinimumJumpDistance / current.StrainTime);
        ...
        foreach (DroidDifficultyHitObject prev in RetrievePastVisibleObjects(current))
        {
            ...
            prevDifficulty *= DifficultyCalculationUtils.Smootherstep(prev.LazyJumpDistance, 15, DistanceInfluenceThreshold);
            prevDifficulty *= GetTimeNerfFactor(current.StartTime - prev.StartTime);
            pastObjectDifficultyInfluence += prevDifficulty;
        }

        double noteDensityDifficulty = System.Math.Pow(pastObjectDifficultyInfluence, 1.45) * 0.9 * constantAngleNerfFactor * velocityFactor;
        ...
        if (mods.Any(static mod => mod is ModHidden))
        {
            ...
            double densityFactor = System.Math.Pow(System.Math.Max(1d, futureObjectDifficultyInfluence + pastObjectDifficultyInfluence - 2), 2.3) * 3.2;
            ...
            if (current.LazyJumpDistance == 0d &&
                current.OpacityAt(prev.Obj.StartTime + prev.Obj.TimePreempt, mods) == 0d &&
                prev.StartTime + prev.TimePreempt > current.StartTime)
            {
                hiddenDifficulty += (HiddenMultiplier * 1303) / System.Math.Pow(current.StrainTime, 1.5);
            }
        }

        double preemptDifficulty =
            System.Math.Pow((PreemptStartingPoint - current.TimePreempt + System.Math.Abs(current.TimePreempt - PreemptStartingPoint)) / 2d, 2.35) /
        ...
        if (current.Obj is Slider slider)
        {
            ...
            sliderDifficulty += System.Math.Min(4d, currentVelocity * 0.8) * (spanTravelDistance / 125d);
    ...
    private static IEnumerable<DroidDifficultyHitObject> RetrievePastVisibleObjects(DroidDifficultyHitObject current)
    {
        for (int i = 0; i < current.Index; ++i)
        {
            ...
            if (current.StartTime - prev.StartTime > ReadingWindowSize || prev.StartTime + prev.TimePreempt < current.StartTime)
            {
                yield break;
            }
    ...
    private static double CalculateCurrentVisibleObjectsDensity(DroidDifficultyHitObject current)
    {
        ...
        while (next is not null)
        {
            ...
            if (timeDifference > ReadingWindowSize || current.StartTime + current.TimePreempt < next.StartTime)
            {
                break;
            }
    ...
    private static double GetConstantAngleNerfFactor(DroidDifficultyHitObject current)
    {
        ...
        while (currentTimeGap < maxTimeLimit)
        {
            ...
            if (loopObj.Angle.HasValue && current.Angle.HasValue)
            {
                ...
                double longIntervalFactor = System.Math.Clamp(1 - (loopObj.StrainTime - minTimeLimit) / (maxTimeLimit - minTimeLimit), 0d, 1d);
                constantAngleCount += System.Math.Cos(3 * System.Math.Min(System.Math.PI / 6, angleDifference)) * longIntervalFactor;
            }
