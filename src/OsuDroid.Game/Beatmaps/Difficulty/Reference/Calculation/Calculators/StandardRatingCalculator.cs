    ...
    double sliderFactor)
{
    public double ComputeAimRating(double aimDifficultyValue)
    {
        ...
        double approachRateLengthBonus = 0.95 + 0.4 * System.Math.Min(1d, totalHits / 2000d) + (totalHits > 2000 ? System.Math.Log10(totalHits / 2000d) * 0.5 : 0);
        double approachRateFactor = HasMod<ModRelax>()
            ? 0d
            : approachRate > 10.33
        ...
        ratingMultiplier *= 0.98 + System.Math.Pow(System.Math.Max(0d, overallDifficulty), 2) / 2500;
    ...
    public double ComputeSpeedRating(double speedDifficultyValue)
    {
        ...
        double ratingMultiplier = 1d;
        double approachRateLengthBonus = 0.95 + 0.4 * System.Math.Min(1d, totalHits / 2000d) + (totalHits > 2000 ? System.Math.Log10(totalHits / 2000d) * 0.5 : 0);
        double approachRateFactor = HasMod<ModAutopilot>() ? 0d : approachRate > 10.33 ? 0.3 * (approachRate - 10.33) : 0d;
    ...
    public double ComputeFlashlightRating(double flashlightDifficultyValue)
    {
        ...
        double ratingMultiplier = 0.7 + 0.1 * System.Math.Min(1d, totalHits / 200d) + (totalHits > 200 ? 0.2 * System.Math.Min(1d, (totalHits - 200) / 200d) : 0d);
        ratingMultiplier *= 0.98 + System.Math.Pow(System.Math.Max(0d, overallDifficulty), 2) / 2500;
    ...
    private double CalculateAimVisibilityFactor()
    {
        ...
        double mechanicalDifficultyFactor = Interpolation.ReverseLinear(mechanicalDifficultyRating, 5, 10);
    ...
    private bool HasMod<TMod>() where TMod : Mod => mods.Any(static mod => mod is TMod);
        ...
        double sliderFactor = 1d)
    {
        ...
        bool alwaysPartiallyVisible = hidden?.OnlyFadeApproachCircles ?? mods.Any(static mod => mod is ModTraceable);
        double readingBonus = (alwaysPartiallyVisible ? 0.025 : 0.04) * (12 - System.Math.Max(approachRate, 7));
        readingBonus *= visibilityFactor;

        double sliderVisibilityFactor = System.Math.Pow(sliderFactor, 3);
        if (approachRate < 7)
        {
            readingBonus += (alwaysPartiallyVisible ? 0.02 : 0.045) * (7 - System.Math.Max(approachRate, 0)) * sliderVisibilityFactor;
        }

        if (approachRate < 0)
        {
            readingBonus += (alwaysPartiallyVisible ? 0.01 : 0.1) * (1 - System.Math.Pow(1.5, approachRate)) * sliderVisibilityFactor;
        }
