...
#pragma warning disable IDE0072 // Fallback arms are intentional for scene state defaults.
    ...
    private static UiRect GetCenteredLogoBounds(VirtualViewport viewport)
    {
        ...
        return new UiRect(
            (viewport.VirtualWidth - logoSize) / 2f,
            (viewport.VirtualHeight - logoSize) / 2f,
            logoSize,
            logoSize
        );
    }

    private static UiRect GetExpandedLogoBounds() =>
        ReferenceRect(
            ExpandedLogoReferenceX,
            ExpandedLogoReferenceY,
            ExpandedLogoReferenceSize,
            ExpandedLogoReferenceSize
        );

    private float GetMenuTransitionProgress() =>
        _menuVisibility switch
        {
            MenuVisibility.Collapsed => 0f,
            MenuVisibility.Expanding => EaseOutExpo(
                (float)Math.Clamp(_transitionMilliseconds / MenuExpandDurationMilliseconds, 0d, 1d)
            ),
            MenuVisibility.Expanded => 1f,
            MenuVisibility.Collapsing => 1f
                - EaseOutBounce(
                    (float)
                        Math.Clamp(
                            _transitionMilliseconds / MenuCollapseDurationMilliseconds,
                            0d,
                            1d
                        )
                ),
    ...
    private float GetMenuButtonAlpha() =>
        _menuVisibility switch
        {
            MenuVisibility.Collapsed => 0f,
            MenuVisibility.Expanding => 0.9f
                * EaseOutCubic(
                    (float)
                        Math.Clamp(_transitionMilliseconds / MenuExpandDurationMilliseconds, 0d, 1d)
                ),
            MenuVisibility.Expanded => 0.9f,
            MenuVisibility.Collapsing => 0.9f
                * (
                    1f
                    - EaseOutExpo(
                        (float)
    ...
    private float GetAnimatedMenuButtonX(float finalX)
    {
        return _menuVisibility switch
        {
            ...
            MenuVisibility.Expanding => Lerp(
                finalX - ButtonExpandOffset / MainMenuReferenceToVirtualScale,
                finalX,
                EaseOutElastic((float)Math.Clamp(_transitionMilliseconds / 500d, 0d, 1d))
            ),
            MenuVisibility.Expanded => finalX,
            MenuVisibility.Collapsing => Lerp(
                finalX,
                finalX - ButtonCollapseOffset / MainMenuReferenceToVirtualScale,
                EaseOutExpo(
                    (float)
                        Math.Clamp(
                            _transitionMilliseconds / MenuCollapseDurationMilliseconds,
    ...
    private float GetLogoHeartbeatScale()
    {
        ...
        float shrinkProgress = (float)
            Math.Clamp((_heartbeatMilliseconds - growDuration) / shrinkDuration, 0d, 1d);
        return Lerp(LogoBeatScale, 1f, shrinkProgress);
    }

    private float GetExitProgress() =>
    ...
    private static UiRect CreateVersionPillBounds(VirtualViewport viewport, string text)
    {
        ...
        return new UiRect(
            VersionPillMargin,
            viewport.VirtualHeight - height - VersionPillMargin,
            width,
            height
        );
    }

    private static float EstimateVersionTextWidth(string text) =>
        Math.Max(120f, text.Length * VersionPillTextSize * 0.58f + 4f);

    private static UiRect ReferenceRect(float x, float y, float width, float height) =>
        new(
    ...
    private static float EaseOutBounce(float progress)
    {
        ...
        return progress < 1f / divisor ? bounce * progress * progress
            : progress < 2f / divisor ? bounce * (progress -= 1.5f / divisor) * progress + 0.75f
