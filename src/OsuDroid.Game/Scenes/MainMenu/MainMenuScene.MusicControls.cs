...
public sealed partial class MainMenuScene
{
        ...
        elements.Add(
            new UiElementSnapshot(
                ...
                MeasuredTextAnchor: string.IsNullOrWhiteSpace(title)
                    ? null
                    : new UiMeasuredTextAnchor(
                        title,
                        titleStyle,
                        MusicNowPlayingTextRight,
                        MusicNowPlayingSpriteLeftPadding
                    )
            )
        );
    ...
    public static string TruncateNowPlayingTitle(string title)
    {
        ...
        return title.Length > MusicNowPlayingCharactersMaximum
            ? string.Concat(
                title.AsSpan(0, MusicNowPlayingCharactersMaximum - ellipsis.Length),
                ellipsis
            )
            : title;
    }

    private void AddMusicControl(
    ...
    private float GetMusicProgressRatio()
    {
        return _nowPlaying.LengthMilliseconds <= 0
            ? 0f
            : Math.Clamp(
                _nowPlaying.PositionMilliseconds / (float)_nowPlaying.LengthMilliseconds,
                0f,
                1f
            );
    }
