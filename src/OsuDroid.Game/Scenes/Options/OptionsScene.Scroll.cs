...
{
    public static float MaxScrollOffset(VirtualViewport viewport) =>
    ...
    public static bool IsSectionScrollPoint(UiPoint point) =>
        point.X is >= ContentPaddingX and <= (ContentPaddingX + SectionRailWidth);

    public void Scroll(float deltaY, VirtualViewport viewport) =>
        Scroll(
            deltaY,
            new UiPoint(ContentPaddingX + SectionRailWidth + ListGap, ContentTop),
            viewport
        );
    ...
    public void Scroll(float deltaY, UiPoint point, VirtualViewport viewport)
    {
        ...
        {
            _sectionScrollOffset = Math.Clamp(
                _sectionScrollOffset + deltaY,
                0f,
                MaxSectionScrollOffset(viewport)
            );
        }
        else
        {
            _contentScrollOffset = Math.Clamp(
                _contentScrollOffset + deltaY,
                0f,
                MaxActiveContentScrollOffset(viewport)
            );
        }
    }
    ...
    )
    {
        ...
        {
            OptionsScrollTarget.Sections => _sectionScroll.Drag(
                point,
                timestamp,
            ...
            OptionsScrollTarget.Content => _contentScroll.Drag(
                point,
                timestamp,
    ...
    private float MaxActiveContentScrollOffset(VirtualViewport viewport) =>
        Math.Max(
            0f,
            CalculateContentHeight(ActiveSectionData.Categories) - VisibleContentHeight(viewport)
        );
    ...
    private static bool TryParseSliderRowIndex(string elementId, out int rowIndex)
    {
        ...
        return suffixIndex < 0
            ? false
            : int.TryParse(
                elementId.AsSpan(prefix.Length, suffixIndex - prefix.Length),
                NumberStyles.None,
