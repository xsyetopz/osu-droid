...
{
    private UiFrameSnapshot CreateUiFrame(
    ...
    private void AddSections(
        ...
        VirtualViewport viewport,
        OptionsSection selectedSectionValue,
        float activeSectionScrollOffset
    )
    {
        for (int i = 0; i < OptionsCatalog.Sections.Length; i++)
        {
            ...
            elements.Add(
                Text(
                    $"options-section-{i}-text",
                    _localizer[section.Key],
                    textX,
                    bounds.Y + (bounds.Height - RowTitleSize) / 2f,
                    bounds.Right - textX - SectionPadding,
                    RowTitleSize + 4f,
                    RowTitleSize,
                    textColor,
                    isSelected ? 1f : 0.9f,
    ...
    )
    {
        ...
        foreach (SettingsCategory category in sectionData.Categories)
        {
            ...
            {
                var categoryHeaderBounds = new UiRect(
                    listX,
                    categoryTop,
                    listWidth,
                ...
                elements.Add(
                    Fill(
                        $"options-category-{rowIndex}-header",
    ...
    private void AddStatusMessage(List<UiElementSnapshot> elements, VirtualViewport viewport)
    {
        ...
        var bounds = new UiRect(
            ...
            viewport.VirtualHeight - 72f * DpScale,
            width,
            44f * DpScale
        );
        elements.Add(
            Fill(
                "options-status-message-bg",
                bounds,
                s_selectedSection,
                0.96f,
                UiAction.None,
        ...
        elements.Add(
            Text(
                ...
                message,
                bounds.X + 18f * DpScale,
                bounds.Y + 10f * DpScale,
                bounds.Width - 36f * DpScale,
                RowTitleSize + 4f,
                RowTitleSize,
                s_white,
                1f,
