...
public sealed class StartupScene
{
    ...
    private const double LoadingFadeOutMilliseconds =
        DroidUiTimings.StartupLoadingFadeOutMilliseconds;
    private const double WelcomeDelayMilliseconds = DroidUiTimings.StartupWelcomeDelayMilliseconds;
    private const double WelcomeStretchMilliseconds =
        DroidUiTimings.StartupWelcomeStretchMilliseconds;
    ...
    public bool IsComplete =>
        _elapsedMilliseconds >= WelcomeStartMilliseconds + WelcomeMilliseconds;
    ...
    public void Update(TimeSpan elapsed) =>
        _elapsedMilliseconds = Math.Min(
            ...
            WelcomeStartMilliseconds + WelcomeMilliseconds
        );
    ...
    private void AddWelcome(List<UiElementSnapshot> elements, VirtualViewport viewport)
    {
        ...
        float yScale =
            welcomeElapsed < WelcomeStretchMilliseconds
                ...
                : 1f
                    + 0.1f
                        * (float)
                            Math.Clamp(
                                (welcomeElapsed - WelcomeStretchMilliseconds)
                                    / (WelcomeMilliseconds - WelcomeStretchMilliseconds),
                                0d,
                                1d
                            );
        UiAssetEntry asset = DroidAssets.StartupManifest.Get(DroidAssets.Welcome);
        float width =
            asset.NativeSize.Width * (welcomeElapsed < WelcomeStretchMilliseconds ? 1f : yScale);
    ...
    private float LoadingAlpha =>
        1f - (float)Math.Clamp(_elapsedMilliseconds / LoadingFadeOutMilliseconds, 0d, 1d);
}
