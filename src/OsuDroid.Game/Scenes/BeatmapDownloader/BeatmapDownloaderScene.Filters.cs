...
public sealed partial class BeatmapDownloaderScene
{
    private void AddFilterPanel(List<UiElementSnapshot> elements, VirtualViewport viewport)
    {
        ...
        string orderText = _order == BeatmapMirrorOrder.Ascending ? _localizer["BeatmapDownloader_Ascending"] : _localizer["BeatmapDownloader_Descending"];
        string statusLabel = _localizer["BeatmapDownloader_Status"];
        string statusText = _status is null ? _localizer["Common_All"] : RankedStatusText(_status.Value);
        float x = 12f * Dp;
        ...
        var orderBounds = new UiRect(orderLabelX + orderLabelWidth + labelGap, buttonTop, orderWidth, 42f * Dp);
        float statusLabelX = orderBounds.Right + groupGap;
        var statusBounds = new UiRect(statusLabelX + statusLabelWidth + labelGap, buttonTop, statusWidth, 42f * Dp);
        elements.Add(Text("downloader-filter-sort-label", sortLabel, x, labelTop, sortLabelWidth, 22f * Dp, 13f * Dp, s_filterLabel));
        AddDropdownButton(elements, "downloader-filter-sort", sortBounds, sortText, UiAction.DownloaderSort);
        elements.Add(Text("downloader-filter-order-label", orderLabel, orderLabelX, labelTop, orderLabelWidth, 22f * Dp, 13f * Dp, s_filterLabel));
        AddButton(elements, "downloader-filter-order", orderBounds, orderText, UiAction.DownloaderOrder);
        elements.Add(Text("downloader-filter-status-label", statusLabel, statusLabelX, labelTop, statusLabelWidth, 22f * Dp, 13f * Dp, s_filterLabel));
        AddDropdownButton(elements, "downloader-filter-status", statusBounds, statusText, UiAction.DownloaderStatus);
        if (_sortDropdownOpen)
        {
            AddDropdownOptions(elements, "downloader-sort-option", sortBounds.X, sortBounds.Y, sortBounds.Width, SortOptions(), SortAction(_sort), _sortDropdownScroll, viewport);
        }

        if (_statusDropdownOpen)
        {
            AddDropdownOptions(elements, "downloader-status-option", statusBounds.X, statusBounds.Y, statusBounds.Width, StatusOptions(), StatusAction(_status), _statusDropdownScroll, viewport);
        }
    }
    ...
    private static UiAction SortAction(BeatmapMirrorSort sort) => sort switch
    {
    ...
    private static UiAction StatusAction(BeatmapRankedStatus? status) => status switch
    {
    ...
    private static void AddDropdownOptions(List<UiElementSnapshot> elements, string id, float x, float y, float width, (string Text, UiAction Action)[] actions, UiAction selectedAction, float scroll, VirtualViewport viewport)
    {
        float rowHeight = 42f * Dp;
        float padding = 8f * Dp;
        float availableHeight = Math.Max(rowHeight, viewport.VirtualHeight - y);
        float rowStep = rowHeight + 4f * Dp;
        float panelWidth = DropdownWidth(width, actions, Math.Max(width, viewport.VirtualWidth - x));
        float height = Math.Min(actions.Length * rowStep + padding * 2f, availableHeight);
        elements.Add(Fill(id + "-panel", new UiRect(x, y, panelWidth, height), s_dropdownPanel, 1f, UiAction.None, Radius));
        ...
        for (int i = first; i < actions.Length; i++)
        {
            ...
            AddDropdownOption(elements, $"{id}-{i}", new UiRect(x + padding, rowY, panelWidth - padding * 2f, rowHeight), actions[i].Text, actions[i].Action, actions[i].Action == selectedAction);
        }
    }
    ...
    private void AddMirrorDialog(List<UiElementSnapshot> elements, VirtualViewport viewport)
    {
        ...
        elements.Add(Fill("downloader-mirror-scrim", new UiRect(0f, 0f, viewport.VirtualWidth, viewport.VirtualHeight), s_dialogScrim, 1f, UiAction.DownloaderMirror));
        elements.Add(Fill("downloader-mirror-dialog", new UiRect(x, y, width, height), s_appBar, 1f, UiAction.DownloaderDetailsPanel, Radius));
        elements.Add(TextMiddle("downloader-mirror-title", _localizer["BeatmapDownloader_SelectMirror"], x, y, width, titleHeight, 14f * Dp, s_secondary, UiTextAlignment.Center, UiAction.DownloaderDetailsPanel));
        elements.Add(Fill("downloader-mirror-divider", new UiRect(x, y + titleHeight, width, dividerHeight), s_background, 0.45f, UiAction.DownloaderDetailsPanel));
        float rowX = x + padding;
        float rowWidth = width - padding * 2f;
        float firstRowY = y + titleHeight + dividerHeight + padding;
        AddMirrorOption(elements, BeatmapMirrorKind.OsuDirect, new UiRect(rowX, firstRowY, rowWidth, rowHeight), UiAction.DownloaderMirrorOsuDirect);
        AddMirrorOption(elements, BeatmapMirrorKind.Catboy, new UiRect(rowX, firstRowY + rowHeight, rowWidth, rowHeight), UiAction.DownloaderMirrorCatboy);
    }

    private void AddMirrorOption(List<UiElementSnapshot> elements, BeatmapMirrorKind kind, UiRect bounds, UiAction action)
    {
        ...
        if (kind == _mirror)
        {
            elements.Add(MaterialIcon($"downloader-mirror-{kind}-check", UiMaterialIcon.Check, new UiRect(bounds.Right - 36f * Dp, bounds.Y + 17f * Dp, 24f * Dp, 24f * Dp), s_accent, 1f, action));
        }
    }

    private static float LabelWidth(string text) => EstimateTextWidth(text, 13f * Dp) + 12f * Dp;

    private static float DropdownWidth(float callerWidth, (string Text, UiAction Action)[] actions, float maxWidth)
    {
        float widestText = actions.Length == 0 ? 0f : actions.Max(action => EstimateTextWidth(action.Text, 14f * Dp));
        float naturalWidth = widestText + 64f * Dp;
        return MathF.Ceiling(Math.Clamp(Math.Max(callerWidth, naturalWidth), callerWidth, maxWidth));
    }
