...
public sealed partial class BeatmapDownloaderScene
{
    private void AddDetails(List<UiElementSnapshot> elements, VirtualViewport viewport)
    {
        ...
        float panelHeight = Math.Min(
            coverHeight + difficultyRowHeight + footerHeight,
            viewport.VirtualHeight - panelMargin * 2f
        );
        float x = (viewport.VirtualWidth - panelWidth) / 2f;
        float y = Math.Clamp(
            (viewport.VirtualHeight - panelHeight) / 2f,
            panelMargin,
            viewport.VirtualHeight - panelMargin - panelHeight
        );
        elements.Add(
            Fill(
                "downloader-details-panel",
                new UiRect(x, y, panelWidth, panelHeight),
                s_panel,
                1f,
        ...
        AddCover(
            elements,
            "downloader-details-cover",
            set,
            new UiRect(x, y, panelWidth, coverHeight),
            UiAction.DownloaderDetailsPanel
        );
        elements.Add(
            Fill(
                "downloader-details-cover-dim",
                new UiRect(x, y, panelWidth, coverHeight),
                s_coverFallback,
                0.2f,
                UiAction.DownloaderDetailsPanel,
        ...
        elements.Add(
            Text(
                "downloader-details-creator",
                _localizer.Format("BeatmapDownloader_MappedBy", set.Creator),
                x + 16f * Dp,
                y + 68f * Dp,
                panelWidth - 120f * Dp,
                20f * Dp,
        ...
        AddStatusPill(
            elements,
            "downloader-details-status",
            set.Status,
            x + panelWidth - StatusPillWidth(set.Status) - 16f * Dp,
            y + 16f * Dp,
            UiAction.DownloaderDetailsPanel
        );
        ...
        elements.Add(
            Fill(
                "downloader-details-body",
                new UiRect(x, bodyY, panelWidth, footerHeight),
                s_footer,
                1f,
                UiAction.DownloaderDetailsPanel
            )
        ...
        elements.Add(
            Text(
                ...
                _localizer.Format(
                    ...
                    TimeSpan
                        .FromSeconds(beatmap.HitLength)
                        .ToString("m\\:ss", CultureInfo.InvariantCulture),
                    beatmap.Bpm.ToString("0.##", CultureInfo.InvariantCulture)
                ),
                x + 16f * Dp,
                bodyY + 38f * Dp,
                panelWidth - 32f * Dp,
        ...
        {
            new(
                "downloader-details-preview",
                _previewingSetIndex == index ? UiMaterialIcon.Pause : UiMaterialIcon.PlayArrow,
                string.Empty,
                52f * Dp,
                UiAction.DownloaderDetailsPreview
            ),
            new(
                "downloader-details-download",
                UiMaterialIcon.Download,
                _localizer["BeatmapDownloader_Download"],
                112f * Dp,
                UiAction.DownloaderDetailsDownload
            ),
        };
        if (MirrorDefinition(set.Mirror).SupportsNoVideoDownloads && set.HasVideo)
        {
            buttons.Add(
                new DownloaderButtonSpec(
                    "downloader-details-no-video",
                    UiMaterialIcon.Download,
                    _localizer["BeatmapDownloader_DownloadNoVideo"],
                    174f * Dp,
                    UiAction.DownloaderDetailsDownloadNoVideo
                )
            );
        }
    ...
    private void AddDownloadOverlay(List<UiElementSnapshot> elements, VirtualViewport viewport)
    {
        ...
        string text =
            FormatDownloadText(state)
            + (
                progress?.Phase == BeatmapDownloadPhase.Downloading
                    ? FormatDownloadInfo(progress)
                    : string.Empty
            );
        ...
        float width = Math.Clamp(
            textWidth + 112f * Dp,
            300f * Dp,
            Math.Min(500f * Dp, viewport.VirtualWidth - 80f * Dp)
        );
        ...
        elements.Add(
            Fill(
                "downloader-download-divider-title",
                new UiRect(x, y + titleHeight, width, 1f * Dp),
                s_background,
                0.45f
            )
        );
        ...
        float sweepDegrees = progress?.Percent is double percent
            ? Math.Clamp((float)(percent * 3.6d), 1f, 360f)
            : 96f;
        float rotationDegrees = progress?.Percent is double ? 0f : _loadingIndicatorRotationDegrees;
        elements.Add(
            ProgressRing(
                "downloader-download-spinner",
                new UiRect(
                    contentX + 9f * Dp,
                    bodyY + (bodyHeight - 24f * Dp) / 2f,
                    24f * Dp,
                    24f * Dp
                ),
                s_accent,
        ...
        elements.Add(
            Fill(
                "downloader-download-divider-cancel",
                new UiRect(x, bodyY + bodyHeight, width, 1f * Dp),
                s_background,
                0.45f
            )
        );
        ...
        elements.Add(
            Fill(
                "downloader-download-cancel-hit",
                new UiRect(x, cancelY, width, cancelHeight),
                s_field,
                1f,
                UiAction.DownloaderDownloadCancel,
                Radius
            )
        );
        ...
        elements.Add(
            UiElementFactory.Icon(
                "downloader-download-cancel-icon",
                UiIcon.Close,
                new UiRect(cancelContentX, cancelY + 9f * Dp, 24f * Dp, 24f * Dp),
                DroidUiTheme.Controls.Danger,
                1f,
                UiAction.DownloaderDownloadCancel
            )
        );
        elements.Add(
            TextMiddle(
                "downloader-download-cancel-text",
                _localizer["BeatmapDownloader_Cancel"],
                cancelContentX + 30f * Dp,
                cancelY,
                cancelContentWidth - 30f * Dp,
                cancelHeight,
                14f * Dp,
                DroidUiTheme.Controls.Danger,
                UiTextAlignment.Left,
