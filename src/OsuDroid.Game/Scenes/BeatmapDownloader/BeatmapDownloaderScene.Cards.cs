...
public sealed partial class BeatmapDownloaderScene
{
    private void AddCards(List<UiElementSnapshot> elements, VirtualViewport viewport)
    {
        ...
        if (_isSearching)
        {
            elements.Add(
                Text(
                    "downloader-loading-more",
                    _localizer["BeatmapDownloader_LoadingMore"],
                    0f,
                    viewport.VirtualHeight - 38f * Dp,
    ...
    )
    {
        ...
        elements.Add(
            Fill(
                $"downloader-card-{slot}",
                new UiRect(x, y, CardWidth, CardHeight),
                s_panel,
                1f,
                cardAction,
                Radius
        ...
        AddCover(
            elements,
            $"downloader-card-{slot}-cover",
            set,
            new UiRect(x, y, CardWidth, CardTopHeight),
            cardAction
        );
        elements.Add(
            Fill(
                $"downloader-card-{slot}-cover-dim",
                new UiRect(x, y, CardWidth, CardTopHeight),
                s_coverFallback,
                0.2f,
                cardAction,
        ...
        AddDifficultyDots(
            elements,
            $"downloader-card-{slot}-diff",
            set.Beatmaps,
            x + 8f * Dp,
            y + CardTopHeight + (CardDifficultyHeight - DifficultyGlyphRowHeight) / 2f,
            CardWidth - 16f * Dp,
            null,
            true,
            cardAction
        ...
        elements.Add(
            Fill(
                $"downloader-card-{slot}-footer",
                new UiRect(x, footerY, CardWidth, CardFooterHeight),
                s_footer,
                1f,
                cardAction
            )
        ...
        {
            new(
                $"downloader-card-{slot}-preview",
                _previewingSetIndex == absoluteIndex
                    ? UiMaterialIcon.Pause
                    : UiMaterialIcon.PlayArrow,
                string.Empty,
                52f * Dp,
                previewAction
            ),
            new(
                $"downloader-card-{slot}-download",
