...
public sealed partial class BeatmapDownloaderScene
{
    public static int DownloadIndex(UiAction action)
    {
        ...
        return index >= 0 ? index : IndexedActionOffset(action, UiAction.DownloaderDownloadNoVideo0, 8);
    }
    ...
    public static int CardIndex(UiAction action) => IndexedActionOffset(action, UiAction.DownloaderCard0, 8);

    public static int PreviewIndex(UiAction action) => IndexedActionOffset(action, UiAction.DownloaderPreview0, 8);

    public static int DifficultyIndex(UiAction action) => IndexedActionOffset(action, UiAction.DownloaderDetailsDifficulty0, 16);

    private static UiAction CardAction(int index) => IndexedAction(UiAction.DownloaderCard0, index, 8);

    private static UiAction PreviewAction(int index) => IndexedAction(UiAction.DownloaderPreview0, index, 8);

    private static UiAction DownloadAction(int index) => IndexedAction(UiAction.DownloaderDownload0, index, 8);

    private static UiAction NoVideoAction(int index) => IndexedAction(UiAction.DownloaderDownloadNoVideo0, index, 8);

    private static UiAction DifficultyAction(int index) => IndexedAction(UiAction.DownloaderDetailsDifficulty0, index, 16);
