...
{
    public static UiElementSnapshot ModalScrim(string id, UiRect bounds, UiAction action = UiAction.None, UiColor? color = null) =>
    ...
    public static UiElementSnapshot Panel(
        ...
        UiAction action = UiAction.None) => UiElementFactory.Fill(
            id,
            bounds,
            color ?? DroidUiColors.SurfaceRow,
            alpha,
            action,
            radius);
        ...
        bool isEnabled = true)
    {
        ...
        elements.Add(UiElementFactory.Fill(
            ...
            radius,
            isEnabled));
        elements.Add(UiElementFactory.Text(
            id + "-text",
            text,
            bounds,
            textSize,
    ...
    {
        UiColor resolvedForeground = foreground ?? DroidUiColors.TextPrimary;
        elements.Add(UiElementFactory.Fill(id + "-background", bounds, background ?? DroidUiColors.SurfaceSelected, 1f, action, radius));

        float textWidth = string.IsNullOrEmpty(text) ? 0f : MathF.Max(1f, text.Length * textSize * 0.55f);
        float gap = textWidth > 0f ? 8f : 0f;
        float contentWidth = iconSize + gap + textWidth;
        float x = bounds.X + (bounds.Width - contentWidth) / 2f;
        var iconBounds = new UiRect(x, bounds.Y + (bounds.Height - iconSize) / 2f, iconSize, iconSize);
        elements.Add(UiElementFactory.MaterialIcon(id + "-icon", icon, iconBounds, resolvedForeground, 1f, action));
        ...
        UiColor? foreground = null)
    {
        if (isSelected)
        {
            elements.Add(UiElementFactory.Fill(id + "-selected", bounds, selectedBackground ?? DroidUiColors.DropdownSelected, 1f, action, DroidUiMetrics.AndroidRoundedRectRadius));
        }

        elements.Add(UiElementFactory.Text(
            id + "-text",
            text,
            new UiRect(bounds.X + 12f, bounds.Y, Math.Max(1f, bounds.Width - 48f), bounds.Height),
            18f,
            foreground ?? DroidUiColors.TextPrimary,
            action,
            alignment: UiTextAlignment.Left,
    ...
    {
        elements.Add(UiElementFactory.Fill(id + "-background", bounds, background, 1f, action, DroidUiMetrics.AndroidRoundedRectRadius));
        elements.Add(UiElementFactory.Text(
            id + "-text",
            text,
            bounds,
    ...
    public static void AddStatusPill(
        ...
        UiColor? background = null)
    {
        elements.Add(UiElementFactory.Fill(id + "-background", bounds, background ?? DroidUiColors.SurfaceRow, 1f, action, DroidUiMetrics.AndroidRoundedRectRadius));
        elements.Add(UiElementFactory.Text(
            id + "-text",
            text,
            bounds,
