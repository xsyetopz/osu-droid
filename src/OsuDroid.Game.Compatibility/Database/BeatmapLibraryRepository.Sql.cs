...
public sealed partial class BeatmapLibraryRepository
{
    ...
    )
    {
        ...
        command.Parameters.AddWithValue(
            "$id",
            beatmap.Id is null ? DBNull.Value : beatmap.Id.Value
        );
        command.Parameters.AddWithValue("$audioFilename", beatmap.AudioFilename);
        command.Parameters.AddWithValue(
            "$backgroundFilename",
            beatmap.BackgroundFilename is null ? DBNull.Value : beatmap.BackgroundFilename
        );
        command.Parameters.AddWithValue(
            "$status",
            beatmap.Status is null ? DBNull.Value : beatmap.Status.Value
        );
        command.Parameters.AddWithValue(
            "$setId",
            beatmap.SetId is null ? DBNull.Value : beatmap.SetId.Value
        );
        ...
        command.Parameters.AddWithValue(
            "$droidStarRating",
            beatmap.DroidStarRating is null ? DBNull.Value : beatmap.DroidStarRating.Value
        );
        command.Parameters.AddWithValue(
            "$standardStarRating",
            beatmap.StandardStarRating is null ? DBNull.Value : beatmap.StandardStarRating.Value
        );
    ...
    private static void ExecuteSetDirectoryDelete(
        SqliteConnection connection,
        SqliteTransaction transaction,
        string text,
        string setDirectory
    ...
    private static void ExecuteCollectionDelete(
        SqliteConnection connection,
        SqliteTransaction transaction,
        string text,
        string name
    ...
    private static BeatmapInfo ReadBeatmap(SqliteDataReader reader) =>
        new(
