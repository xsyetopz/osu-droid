...
internal sealed partial class MonoGameUiRenderer
{
    private static void DrawSprite(
        ...
        UiRect? explicitSource
    )
    {
        var source = explicitSource is null
            ? fit == UiSpriteFit.Stretch
                ? null
                : CalculateSourceRect(texture, destination, fit)
            : new XnaRect(
    ...
    private static void DrawSpriteClipped(
        SpriteBatch spriteBatch,
        Texture2D texture,
        XnaRect destination,
        XnaRect? clip,
    ...
    )
    {
        ...
        var source = explicitSource is null
            ? fit == UiSpriteFit.Stretch
                ? new XnaRect(0, 0, texture.Width, texture.Height)
                : CalculateSourceRect(texture, destination, fit)
                    ?? new XnaRect(0, 0, texture.Width, texture.Height)
            : new XnaRect(
                (int)MathF.Round(explicitSource.Value.X),
    ...
    private static XnaRect? CalculateSourceRect(
        Texture2D texture,
        XnaRect destination,
        UiSpriteFit fit
    )
