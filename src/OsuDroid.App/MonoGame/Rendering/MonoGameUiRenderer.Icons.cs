...
internal sealed partial class MonoGameUiRenderer
{
        ...
        switch (icon)
        {
            ...
            case UiIcon.Check:
                ...
                DrawLine(
                    ...
                    bounds.X + bounds.Width * 0.42f,
                    bounds.Bottom - bounds.Height * 0.22f,
                    bounds.Right - bounds.Width * 0.14f,
            ...
            case UiIcon.CheckboxChecked:
                ...
                DrawIcon(
                    spriteBatch,
                    Inset(bounds, bounds.Width * 0.12f),
                    XnaColor.Black * 0.75f,
                    UiIcon.Check
                );
                break;
    ...
    private void DrawGrid(SpriteBatch spriteBatch, XnaRect bounds, XnaColor color)
    {
        ...
        for (var x = 0; x < 3; x++)
            DrawFill(
                spriteBatch,
                new XnaRect(startX + x * (cell + gap), startY + y * (cell + gap), cell, cell),
                color,
                0f
            );
    }
    ...
    private void DrawMusic(SpriteBatch spriteBatch, XnaRect bounds, XnaColor color)
    {
        ...
        DrawCircle(
            spriteBatch,
            bounds.X + bounds.Width * 0.45f,
            bounds.Bottom - bounds.Height * 0.25f,
            Math.Max(4, bounds.Width / 6),
            color
        );
    }
    ...
    private void DrawGear(SpriteBatch spriteBatch, XnaRect bounds, XnaColor color)
    {
        DrawBorder(
            spriteBatch,
            Inset(bounds, bounds.Width * 0.2f),
            color,
            Math.Max(2, bounds.Width / 10)
        );
