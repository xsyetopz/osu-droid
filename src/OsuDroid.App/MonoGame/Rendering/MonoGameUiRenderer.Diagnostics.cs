...
internal sealed partial class MonoGameUiRenderer
{
        ...
        switch (element.Kind)
        {
            case UiElementKind.Fill:
                if (element.CornerRadius > 1f && element.CornerMode != UiCornerMode.None)
                    _ = shapeStore.GetRoundedFill(
                        destination.Width,
                        destination.Height,
                        element.CornerRadius * frame.Viewport.Scale,
                        element.CornerMode,
                        ToXnaColor(element.Color, element.Alpha),
                        metrics
                    );
                break;
            ...
            case UiElementKind.Sprite:
                if (element.ExternalAssetPath is not null)
                    assetStore.RequestExternalTexture(
                        element.ExternalAssetPath,
                        destination.Width,
                        destination.Height,
                        metrics
                    );
    ...
    )
    {
        ...
        var texture = shapeStore.GetRoundedFill(
            bounds.Width,
            bounds.Height,
            radius,
            cornerMode,
    ...
    private void DrawBorder(SpriteBatch spriteBatch, XnaRect bounds, XnaColor color, int thickness)
    {
        ...
        spriteBatch.Draw(
            pixel,
            new XnaRect(bounds.X, bounds.Y, bounds.Width, horizontalThickness),
            color
        );
        spriteBatch.Draw(
            pixel,
            new XnaRect(
                bounds.X,
                bounds.Bottom - horizontalThickness,
                bounds.Width,
                horizontalThickness
            ),
            color
        ...
        spriteBatch.Draw(
            pixel,
            new XnaRect(bounds.X, bounds.Y, verticalThickness, bounds.Height),
            color
        );
        spriteBatch.Draw(
            pixel,
            new XnaRect(
                bounds.Right - verticalThickness,
                bounds.Y,
                verticalThickness,
                bounds.Height
            ),
    ...
    private void DrawCornerMarkers(SpriteBatch spriteBatch, XnaRect bounds, XnaColor color)
    {
        ...
        spriteBatch.Draw(
            pixel,
            new XnaRect(bounds.Left, bounds.Top, markerSize, markerSize),
            color
        );
        spriteBatch.Draw(
            pixel,
            new XnaRect(bounds.Right - markerSize, bounds.Top, markerSize, markerSize),
            color
        );
        spriteBatch.Draw(
            pixel,
            new XnaRect(bounds.Left, bounds.Bottom - markerSize, markerSize, markerSize),
            color
        );
        spriteBatch.Draw(
            pixel,
            new XnaRect(
                bounds.Right - markerSize,
                bounds.Bottom - markerSize,
                markerSize,
                markerSize
            ),
            color
    ...
    private void DrawUnderline(SpriteBatch spriteBatch, XnaRect bounds, XnaColor color)
    {
        ...
        spriteBatch.Draw(
            pixel,
            new XnaRect(bounds.X, bounds.Bottom - thickness, bounds.Width, thickness),
            color
        );
    }
}
