...
public sealed partial class UiCompatibilityTests
{
    ...
    public void MobileProjectUsesContentPipelineForStaticDroidImages()
    {
        ...
        string project = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "OsuDroid.App", "OsuDroid.App.csproj")
        );
        string content = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "OsuDroid.App", "Content", "DroidAssets.mgcb")
        );

        Assert.That(
            project,
            Does.Contain("<MonoGameContentReference Include=\"Content\\DroidAssets.mgcb\" />")
        );
        ...
        Assert.That(
            content,
            Does.Contain(
                "/build:../Resources/Raw/assets/droid/main-menu/background.png;droid/main-menu/background"
            )
        );
    }
    ...
    {
        string repositoryRoot = FindRepositoryRoot();
        byte[] appIcon = File.ReadAllBytes(
            Path.Combine(
                repositoryRoot,
                "src",
                "OsuDroid.App",
        ...
        byte[] referenceIcon = File.ReadAllBytes(
            Path.Combine(
                repositoryRoot,
                "third_party",
                "osu-droid-legacy",
    ...
    {
        string repositoryRoot = FindRepositoryRoot();
        string mainActivity = File.ReadAllText(
            Path.Combine(
                repositoryRoot,
                "src",
                "OsuDroid.App",
        ...
        string mainApplication = File.ReadAllText(
            Path.Combine(
                repositoryRoot,
                "src",
                "OsuDroid.App",
        ...
        Assert.That(
            mainApplication,
            Does.Contain("[Application(Icon = \"@drawable/ic_launcher\")]")
        );
        foreach (string? density in densities)
        {
            string androidIcon = Path.Combine(
                repositoryRoot,
                "src",
                "OsuDroid.App",
                "Platforms",
            ...
            string referenceIcon = Path.Combine(
                repositoryRoot,
                "third_party",
                "osu-droid-legacy",
                "res",
            ...
            Assert.That(
                File.ReadAllBytes(androidIcon),
                Is.EqualTo(File.ReadAllBytes(referenceIcon)),
                density
            );
        }
    }
    ...
    public void MobileProjectDeclaresIconSources()
    {
        ...
        string infoPlist = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "OsuDroid.App", "Platforms", "iOS", "Info.plist")
        );

        Assert.That(
            project,
            Does.Contain(
                "<MauiIcon Include=\"Resources\\AppIcon\\appicon.png\" BaseSize=\"192,192\" />"
            )
        );
        ...
        Assert.That(
            project,
            Does.Contain(
                "<BundleResource Include=\"Platforms\\iOS\\Icons\\Icon-60@2x.png\" Link=\"Icon-60@2x.png\" />"
            )
        );
    ...
    public void IosBundleVerifierRequiresDirectAppIcons()
    {
        ...
        string script = File.ReadAllText(
            Path.Combine(repositoryRoot, "scripts", "verify-ios-bundle.sh")
        );
        string infoPlist = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "OsuDroid.App", "Platforms", "iOS", "Info.plist")
        );
        ...
        Assert.That(
            script,
            Does.Contain("Icon-60@2x.png Icon-60@3x.png Icon-76@2x.png Icon-83.5@2x.png")
        );
