...
public sealed partial class BeatmapDownloaderTests
{
    ...
    public void OsuDirectSearchUrlMatchesOsuDroidEndpoint()
    {
        ...
        Uri uri = client.CreateSearchUri(
            new BeatmapMirrorSearchRequest("camellia", Offset: 50, Limit: 25)
        );

        Assert.That(
            uri.GetLeftPart(UriPartial.Path),
            Is.EqualTo("https://osu.direct/api/v2/search")
        );
    ...
    public void OsuDirectDownloadUrlSupportsNoVideo()
    {
        ...
        Assert.That(
            client.CreateDownloadUri(123, true).ToString(),
            Is.EqualTo("https://osu.direct/api/d/123")
        );
        Assert.That(
            client.CreateDownloadUri(123, false).ToString(),
            Is.EqualTo("https://osu.direct/api/d/123?noVideo=1")
        );
        Assert.That(
            client.CreatePreviewUri(BeatmapMirrorKind.OsuDirect, 456).ToString(),
            Is.EqualTo("https://osu.direct/api/media/preview/456")
        );
    }
    ...
    public void CatboyUrlsMatchOsuDroidEndpoints()
    {
        var client = new OsuDirectMirrorClient(new HttpClient(new EmptyHandler()));
        Uri uri = client.CreateSearchUri(
            new BeatmapMirrorSearchRequest(
                "camellia",
                Offset: 50,
                Limit: 25,
                Mirror: BeatmapMirrorKind.Catboy,
                Status: BeatmapRankedStatus.Ranked
            )
        );

        Assert.That(
            uri.GetLeftPart(UriPartial.Path),
            Is.EqualTo("https://catboy.best/api/v2/search")
        );
        Assert.That(uri.Query, Does.Contain("sort=ranked_date%3Adesc"));
        Assert.That(uri.Query, Does.Contain("mode=0"));
        Assert.That(uri.Query, Does.Contain("query=camellia"));
        ...
        Assert.That(
            client.CreateDownloadUri(BeatmapMirrorKind.Catboy, 123, false).ToString(),
            Is.EqualTo("https://catboy.best/d/123")
        );
        Assert.That(
            client.CreatePreviewUri(BeatmapMirrorKind.Catboy, 456).ToString(),
            Is.EqualTo("https://catboy.best/preview/audio/456")
        );
    }
    ...
    {
        var client = new OsuDirectMirrorClient(
            new HttpClient(
                new JsonHandler(
                    """
                    [
        ...
        IReadOnlyList<BeatmapMirrorSet> sets = await client
            .SearchAsync(new BeatmapMirrorSearchRequest("title"), CancellationToken.None)
    ...
    public async Task SearchParserKeepsOnlyOsuStandardDifficulties()
    {
        ...
        IReadOnlyList<BeatmapMirrorSet> sets = await client
            .SearchAsync(new BeatmapMirrorSearchRequest("mixed"), CancellationToken.None)
            .ConfigureAwait(false);
