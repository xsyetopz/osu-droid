...
public sealed partial class BeatmapDownloaderTests
{
    ...
    public void DetailsPanelBlocksShadeCloseAndDifficultyDotsStayInteractive()
    {
        ...
        UiElementSnapshot selectedDifficulty = frame.Elements.Single(element =>
            element.Id == "downloader-details-diff-0-selected"
        );
        UiElementSnapshot selectedGlyph = frame.Elements.Single(element =>
            element.Id == "downloader-details-diff-0"
        );

        Assert.That(
            frame.HitTest(new UiPoint(panel.Bounds.X + 24, panel.Bounds.Y + 24))!.Action,
            Is.EqualTo(UiAction.DownloaderDetailsPanel)
        );
        Assert.That(
            selectedDifficulty.Bounds.Width,
            Is.EqualTo(56f * DroidUiMetrics.DpScale).Within(0.001f)
        );
        Assert.That(
            frame
                .HitTest(
                    new UiPoint(
                        selectedDifficulty.Bounds.X + selectedDifficulty.Bounds.Width / 2,
                        selectedDifficulty.Bounds.Y + selectedDifficulty.Bounds.Height / 2
                    )
                )!
                .Action,
        ...
        Assert.That(
            frame
                .HitTest(
                    new UiPoint(
                        selectedGlyph.Bounds.X + selectedGlyph.Bounds.Width / 2,
                        selectedGlyph.Bounds.Y + selectedGlyph.Bounds.Height / 2
                    )
                )!
                .Action,
            Is.EqualTo(UiAction.DownloaderDetailsDifficultySlot0)
    ...
    public void DetailsPanelUsesOsuDroidWrapContentConstraints()
    {
        ...
        UiElementSnapshot body = frame.Elements.Single(element =>
            element.Id == "downloader-details-body"
        );
        UiElementSnapshot download = frame.Elements.Single(element =>
            element.Id == "downloader-details-download-bg"
        );
        ...
        Assert.That(
            panel.Bounds.Y + panel.Bounds.Height / 2f,
            Is.EqualTo(viewport.VirtualHeight / 2f).Within(0.001f)
        );
        Assert.That(
            panel.Bounds.Bottom,
            Is.LessThanOrEqualTo(viewport.VirtualHeight - 8f * DroidUiMetrics.DpScale + 0.001f)
        );
        Assert.That(body.Bounds.Bottom, Is.EqualTo(panel.Bounds.Bottom).Within(0.001f));
        Assert.That(
            download.Bounds.Bottom,
            Is.LessThanOrEqualTo(panel.Bounds.Bottom - 8f * DroidUiMetrics.DpScale)
        );
    }
    ...
    public void DetailsDifficultyRowIsCenteredAndUsesAndroidPaddedSelectionCell()
    {
        ...
        Assert.That(
            left + (right - left) / 2f,
            Is.EqualTo(panel.Bounds.X + panel.Bounds.Width / 2f).Within(1f)
        );
    }
}
