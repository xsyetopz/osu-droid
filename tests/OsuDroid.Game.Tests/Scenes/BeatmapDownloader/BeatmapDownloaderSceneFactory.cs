...
{
    private static BeatmapDownloaderScene CreateScene(
        ITextInputService? textInput = null,
        IBeatmapDownloadService? downloadService = null,
        string? downloadTracePath = null
    ) =>
        new(
    ...
    private static void SetSets(
        BeatmapDownloaderScene scene,
        IReadOnlyList<BeatmapMirrorSet> sets
    ) =>
        typeof(BeatmapDownloaderScene)
            .GetField("_sets", BindingFlags.Instance | BindingFlags.NonPublic)!
            .SetValue(scene, sets);

    private static void SetSearching(BeatmapDownloaderScene scene, bool isSearching) =>
        typeof(BeatmapDownloaderScene)
            .GetField("_isSearching", BindingFlags.Instance | BindingFlags.NonPublic)!
            .SetValue(scene, isSearching);
    ...
    private static BeatmapMirrorSet CreateSet(
        BeatmapRankedStatus status = BeatmapRankedStatus.Qualified,
        bool hasVideo = false
    ) =>
        new(
    ...
    private static BeatmapMirrorSet CreateSetWithDifficulties(int difficultyCount) =>
        new(
            BeatmapMirrorKind.OsuDirect,
            100,
            "Title",
            ...
            false,
            Enumerable
                .Range(0, difficultyCount)
                .Select(index => new BeatmapMirrorBeatmap(
                    index + 1,
                    $"Diff {index + 1}",
                    0.5f + index * 0.5f,
                    5,
