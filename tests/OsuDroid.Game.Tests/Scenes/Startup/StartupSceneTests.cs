...
public sealed partial class UiCompatibilityTests
{
    ...
    public void StartupSceneShowsOriginalLoadingAndWelcomeTiming()
    {
        ...
        UiElementSnapshot background = loading.Elements.Single(element =>
            element.Id == "startup-background"
        );
        UiElementSnapshot spinner = loading.Elements.Single(element =>
            element.Id == "startup-loading-spinner"
        );
        ...
        Assert.That(
            loading.Elements.Any(element => element.Id == "startup-loading-title"),
            Is.False
        );
        Assert.That(
            loading.Elements.Any(element => element.Id == "startup-loading-text"),
            Is.False
        );
        ...
        Assert.That(
            fadingLoading.Elements.Any(element => element.Id == "startup-welcome"),
            Is.False
        );
        Assert.That(
            fadingLoading.Elements.Any(element => element.Id == "startup-loading-spinner"),
            Is.False
        );
        ...
        scene.Update(
            TimeSpan.FromMilliseconds(
                DroidUiTimings.StartupWelcomeDelayMilliseconds - StartupScene.LoadingMilliseconds
            )
        );
        UiFrameSnapshot welcomeStart = scene.CreateSnapshot(viewport).UiFrame;
        UiElementSnapshot welcome = welcomeStart.Elements.Single(element =>
            element.Id == "startup-welcome"
        );
        ...
        Assert.That(
            welcomeStart.Elements.Any(element => element.Id == "startup-loading-spinner"),
            Is.False
        );
        ...
        scene.Update(
            TimeSpan.FromMilliseconds(DroidUiTimings.StartupWelcomeStretchMilliseconds / 2d)
        );
        UiElementSnapshot stretching = scene
            .CreateSnapshot(viewport)
            .UiFrame.Elements.Single(element => element.Id == "startup-welcome");
        ...
        UiElementSnapshot fullHeight = scene
            .CreateSnapshot(viewport)
            .UiFrame.Elements.Single(element => element.Id == "startup-welcome");
        ...
        UiElementSnapshot finalWelcome = scene
            .CreateSnapshot(viewport)
            .UiFrame.Elements.Single(element => element.Id == "startup-welcome");
    ...
    {
        UiFrameSnapshot frame = BootstrapLoadingScene
            .CreateSnapshot(
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "bootstrap-background").Color,
            Is.EqualTo(UiColor.Opaque(0, 0, 0))
        );
        UiElementSnapshot spinner = frame.Elements.Single(element =>
            element.Id == "bootstrap-loading-spinner"
        );
        Assert.That(spinner.AssetName, Is.EqualTo(DroidAssets.Loading));
        Assert.That(spinner.Bounds.Width, Is.EqualTo(212f * 0.4f).Within(0.01f));
        Assert.That(spinner.Bounds.Height, Is.EqualTo(212f * 0.4f).Within(0.01f));
        Assert.That(
            frame.Elements.Any(element =>
                ...
                && element.AssetName == DroidAssets.LoadingTitle
            ),
            Is.False
        );
        UiElementSnapshot progress = frame.Elements.Single(element =>
            element.Id == "bootstrap-loading-progress"
        );
        ...
        UiElementSnapshot text = frame.Elements.Single(element =>
            element.Id == "bootstrap-loading-text"
        );
    ...
    public void BeatmapProcessingBootstrapLoadingSceneShowsOriginalBeatmapTitle()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "bootstrap-loading-progress").Text,
            Is.EqualTo("42 %")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "bootstrap-loading-text").Text,
            Is.EqualTo("Processing beatmaps...")
        );
    }
}
