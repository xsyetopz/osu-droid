...
public sealed partial class ModSelectSceneTests
{
    ...
    public void SearchDebouncesAndUsesContiguousNameMatch()
    {
        ...
        UiFrameSnapshot beforeDebounce = scene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;
        scene.Update(TimeSpan.FromMilliseconds(200));
        UiFrameSnapshot afterDebounce = scene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;

        Assert.That(
            beforeDebounce.Elements.Any(element => element.Id == "modselect-toggle-NF"),
            Is.True
        );
        Assert.That(
            afterDebounce.Elements.Any(element => element.Id == "modselect-toggle-HR"),
            Is.True
        );
        Assert.That(
            afterDebounce.Elements.Any(element => element.Id == "modselect-toggle-NF"),
            Is.False
        );
    }
    ...
    public void SelectedModsIndicatorScrollsPastEightMods()
    {
        ...
        Assert.That(
            initial.Elements.Any(element => element.Id == "modselect-selected-SY"),
            Is.False
        );
        Assert.That(
            scrolled.Elements.Any(element => element.Id == "modselect-selected-SY"),
            Is.True
        );
    }
    ...
    public void CustomizeEnablesWhenSelectedModHasSettings()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-customize").IsEnabled,
            Is.True
        );
    }
    ...
    public void SelectedModUsesLightCardWithDarkText()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-toggle-DT").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.Accent)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-toggle-name-DT").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.SelectedText)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-toggle-icon-DT").Color,
            Is.EqualTo(DroidUiColors.TextPrimary)
        );
    }
    ...
    public void SearchUsesAndroidInputThemeAndIconAspect()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-search").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.Search)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-search-text").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.SearchPlaceholder)
        );
    ...
    public void ToggleRemovesIncompatibleModsAndFadesBlockedChoices()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-toggle-HT").Alpha,
            Is.EqualTo(0.5f)
        );
    }
    ...
    public void ModMenuScrollShowsTemporaryScrollbars()
    {
        ...
        var verticalScene = new ModSelectScene(
            new MemorySettingsStore(),
            new NoOpTextInputService()
        );
        ...
        Assert.That(
            horizontal.Elements.Any(element => element.Id == "modselect-rail-scrollbar"),
            Is.True
        );
        Assert.That(
            vertical.Elements.Any(element =>
                element.Id.StartsWith("modselect-section-", StringComparison.Ordinal)
                && element.Id.EndsWith("-scrollbar", StringComparison.Ordinal)
            ),
            Is.True
        );
    }
    ...
    public void ModMenuSectionDragContinuesAfterRelease()
    {
        ...
        float hardRockY = afterDrag
            .Elements.Single(element => element.Id == "modselect-toggle-HR")
            .Bounds.Y;
        ...
        Assert.That(
            afterInertia.Elements.Single(element => element.Id == "modselect-toggle-HR").Bounds.Y,
            Is.LessThan(hardRockY)
        );
    }
    ...
    public void ConversionSectionScrollsOnShortViewportAndClampsScrollbarInsideList()
    {
        ...
        UiElementSnapshot scoreV2 = frame.Elements.Single(element =>
            element.Id == "modselect-toggle-V2"
        );
        UiElementSnapshot scrollbar = frame.Elements.Single(element =>
            element.Id
            == "modselect-section-OsuDroidLanguagePack_mod_section_difficulty_conversion-scrollbar"
        );

        Assert.That(
            scoreV2.Bounds.Bottom,
            Is.LessThanOrEqualTo(scrollbar.ClipBounds!.Value.Bottom)
        );
        Assert.That(scrollbar.Bounds.Y, Is.GreaterThanOrEqualTo(scrollbar.ClipBounds.Value.Y));
        Assert.That(
            scrollbar.Bounds.Bottom,
            Is.LessThanOrEqualTo(scrollbar.ClipBounds.Value.Bottom)
        );
    }
    ...
    public void DifficultyIncreaseBottomDoesNotBounceBackOnShortViewport()
    {
        ...
        float traceableBefore = beforeUpdate
            .Elements.Single(element => element.Id == "modselect-toggle-TC")
            .Bounds.Y;
        ...
        Assert.That(
            afterUpdate.Elements.Single(element => element.Id == "modselect-toggle-TC").Bounds.Y,
            Is.EqualTo(traceableBefore)
        );
    }
    ...
    public void FooterUsesSelectedBeatmapStats()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-stat-ar-value").Text,
            Is.EqualTo("7.00")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-stat-ar").Bounds.Y,
            Is.EqualTo(664f)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-stat-ar").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.Badge)
        );
        Assert.That(
            frame
                .Elements.Single(element => element.Id == "modselect-stat-ar-label-bg")
                .Bounds.Width,
            Is.LessThan(
                frame.Elements.Single(element => element.Id == "modselect-stat-ar").Bounds.Width
            )
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-stat-od-value").Text,
            Is.EqualTo("7.00")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-stat-cs-value").Text,
            Is.EqualTo("5.00")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-stat-hp-value").Text,
            Is.EqualTo("7.00")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-stat-bpm-value").Text,
            Is.EqualTo("130")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-star-value").Text,
            Is.EqualTo("3.96")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-ranked-badge").Alpha,
            Is.EqualTo(1f)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-ranked-badge").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.Ranked)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-ranked-badge-text").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.RankedText)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-star-badge").Bounds.Right,
            Is.LessThan(
                frame.Elements.Single(element => element.Id == "modselect-ranked-badge").Bounds.X
            )
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-ranked-badge").Bounds.Right,
            Is.LessThan(
                frame.Elements.Single(element => element.Id == "modselect-stat-score").Bounds.X
            )
        );
    }
    ...
    public void UnrankedModUpdatesRankedBadge()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "modselect-ranked-badge-text").Text,
            Is.EqualTo("Unranked")
        );
    }
    ...
    public void PresetAddPersistsSelectedMods()
    {
        ...
        UiFrameSnapshot frame = restoredScene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;

        Assert.That(
            dialog.Elements.Any(element =>
                element.Id == "modselect-preset-dialog-title" && element.Text == "New mod preset"
            ),
            Is.True
        );
        Assert.That(
            dialog.Elements.Any(element =>
                element.Id == "modselect-preset-dialog-name-label" && element.Text == "Name"
            ),
            Is.True
        );
        Assert.That(
            dialog.Elements.Any(element =>
                element.Id == "modselect-preset-dialog-mod-NF"
                && element.AssetName == DroidAssets.ModNoFail
            ),
            Is.True
        );
        Assert.That(
            dialog.Elements.Single(element => element.Id == "modselect-preset-dialog-save").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.Accent)
        );
        Assert.That(
            frame.Elements.Any(element =>
                element.Id == "modselect-preset-Safe-name" && element.Text == "Safe"
            ),
            Is.True
        );
        Assert.That(
            frame.Elements.Any(element =>
                element.Id == "modselect-preset-Safe-NF"
                && element.AssetName == DroidAssets.ModNoFail
            ),
            Is.True
        );
    ...
    public void PresetRowsToggleSelectionAndLongPressDeletesWithConfirmation()
    {
        ...
        Assert.That(scene.SelectedAcronyms, Does.Contain("DT"));
        Assert.That(
            selected.Elements.Single(element => element.Id == "modselect-preset-Fast").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.Accent)
        );
        Assert.That(deleteOpened, Is.True);
        Assert.That(
            deleteDialog.Elements.Any(element =>
                element.Id == "modselect-preset-delete-title" && element.Text == "Delete preset"
            ),
            Is.True
        );
        Assert.That(
            deleteDialog.Elements.Any(element =>
                element.Id == "modselect-preset-delete-message"
                && element.Text == "Delete preset \"Fast\"?"
            ),
            Is.True
        );
        Assert.That(
            deleted.Elements.Any(element => element.Id == "modselect-preset-Fast"),
            Is.False
        );
    }
        ...
        Assert.That(
            frame
                .Elements.Where(element =>
                    element.Id.StartsWith("modselect-selected-", StringComparison.Ordinal)
                )
                .All(element => element.ClipBounds == new UiRect(506f, 12f, 340f, 58f)),
            Is.True
        );
    }
    ...
    public void SongSelectModsOpensModSelectAndBackReturnsToSongSelect()
    {
        string root = Path.Combine(
            ...
            $"modselect-route-{Guid.NewGuid():N}"
        );
        ...
        Assert.That(
            modFrame.UiFrame.Elements.Any(element => element.Id == "songselect-base"),
            Is.True
        );
        Assert.That(
            modFrame.UiFrame.Elements.Single(element => element.Id == "songselect-mods").Action,
            Is.EqualTo(UiAction.None)
        );
        Assert.That(
            modFrame.UiFrame.Elements.Single(element => element.Id == "modselect-background").Alpha,
            Is.EqualTo(0.9f)
        );
        Assert.That(
            modFrame.UiFrame.Elements.Single(element => element.Id == "modselect-background").Color,
            Is.EqualTo(DroidUiTheme.ModMenu.SelectedText)
        );
