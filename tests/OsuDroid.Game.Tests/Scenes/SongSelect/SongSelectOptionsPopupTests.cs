...
public sealed partial class SongSelectSceneTests
{
    ...
    public void BeatmapOptionsButtonOpensSearchOptionsPopup()
    {
        ...
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-properties-panel"),
            Is.False
        );
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-beatmap-options-search"),
            Is.True
        );
        Assert.That(
            frame
                .Elements.Single(element => element.Id == "songselect-beatmap-options-search-icon")
                .MaterialIcon,
            Is.EqualTo(UiMaterialIcon.Search)
        );
        UiElementSnapshot favoriteIcon = frame.Elements.Single(element =>
            element.Id == "songselect-beatmap-options-favorite-icon"
        );
        ...
        Assert.That(
            frame
                .Elements.Single(element =>
                    element.Id == "songselect-beatmap-options-algorithm-icon"
                )
                .MaterialIcon,
            Is.EqualTo(UiMaterialIcon.StarOutline)
        );
        Assert.That(
            frame
                .Elements.Single(element => element.Id == "songselect-beatmap-options-sort-icon")
                .MaterialIcon,
            Is.EqualTo(UiMaterialIcon.Sort)
        );
        Assert.That(
            frame
                .Elements.Single(element => element.Id == "songselect-beatmap-options-folder-icon")
                .MaterialIcon,
            Is.EqualTo(UiMaterialIcon.FolderOutline)
        );
        Assert.That(
            frame
                .Elements.Single(element => element.Id == "songselect-beatmap-options-folder")
                .Text,
            Is.EqualTo("Default")
        );
    }
    ...
    public void BeatmapOptionsUsesOsuDroidRoundedContainerGraphics()
    {
        ...
        UiElementSnapshot search = frame.Elements.Single(element =>
            element.Id == "songselect-beatmap-options-search"
        );
        UiElementSnapshot strip = frame.Elements.Single(element =>
            element.Id == "songselect-beatmap-options-strip"
        );
        UiElementSnapshot[] hitFills = frame
            .Elements.Where(element =>
                element.Id.StartsWith("songselect-beatmap-options-", StringComparison.Ordinal)
                && element.Id.EndsWith("-hit", StringComparison.Ordinal)
            )
            .ToArray();
        ...
        Assert.That(
            frame.Elements.Count(element =>
                element.Id.StartsWith(
                    "songselect-beatmap-options-divider-",
    ...
    public void BeatmapOptionsTabStripUsesAndroidWrapContentSizing()
    {
        ...
        UiFrameSnapshot frame = scene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;
        UiElementSnapshot favorite = frame.Elements.Single(element =>
            element.Id == "songselect-beatmap-options-favorite-hit"
        );
        UiElementSnapshot algorithm = frame.Elements.Single(element =>
            element.Id == "songselect-beatmap-options-algorithm-hit"
        );
        UiElementSnapshot sort = frame.Elements.Single(element =>
            element.Id == "songselect-beatmap-options-sort-hit"
        );
        UiElementSnapshot folder = frame.Elements.Single(element =>
            element.Id == "songselect-beatmap-options-folder-hit"
        );
        UiElementSnapshot strip = frame.Elements.Single(element =>
            element.Id == "songselect-beatmap-options-strip"
        );
        ...
        Assert.That(
            algorithm.Bounds.Width,
            Is.EqualTo(ExpectedOptionsWidth("osu!droid", 16f)).Within(0.001f)
        );
        Assert.That(
            sort.Bounds.Width,
            Is.EqualTo(ExpectedOptionsWidth("Title", 16f)).Within(0.001f)
        );
        Assert.That(
            folder.Bounds.Width,
            Is.EqualTo(ExpectedOptionsWidth("Default", 24f)).Within(0.001f)
        );
        Assert.That(
            strip.Bounds.Width,
            Is.EqualTo(
                    favorite.Bounds.Width
                        + algorithm.Bounds.Width
                        + sort.Bounds.Width
                        + folder.Bounds.Width
                        + 3f * dp
    ...
    {
        var first = new BeatmapSetInfo(
            1,
            "1 Artist - Title",
            [CreateBeatmap("Easy", null, 1.5f, setDirectory: "1 Artist - Title")]
        );
        var second = new BeatmapSetInfo(
            2,
            "2 Other - Song",
            [
                CreateBeatmap(
        ...
        Assert.That(
            frame.Elements.Any(element =>
                element.Text?.Contains("Artist - Title", StringComparison.Ordinal) == true
            ),
            Is.False
        );
        Assert.That(
            frame.Elements.Any(element =>
                element.Text?.Contains("Other - Song", StringComparison.Ordinal) == true
            ),
            Is.True
        );
        UiElementSnapshot favoriteIcon = frame.Elements.Single(element =>
    ...
    {
        BeatmapInfo filteredOut = CreateBeatmap(
            "Easy",
            null,
            1.5f,
            setDirectory: "1 Artist - Title"
        ) with
        {
        ...
        BeatmapInfo matched = CreateBeatmap(
            "Insane",
            null,
            3.5f,
            setId: 2,
    ...
    {
        BeatmapInfo low = CreateBeatmap(
            "Low",
            null,
            1.5f,
            setDirectory: "1 First",
            title: "First",
            artist: "Artist"
        ) with
        {
        ...
        BeatmapInfo high = CreateBeatmap(
            "High",
            null,
            4.5f,
            setDirectory: "1 First",
        ...
        var scene = new SongSelectScene(
            new FakeLibrary(
                new BeatmapLibrarySnapshot([new BeatmapSetInfo(1, "1 First", [low, high])])
            ),
            new NoOpMenuMusicController(),
            new FakeDifficultyService(),
            CreateSongsRoot("audio.mp3")
        ...
        UiFrameSnapshot droidStarsFrame = scene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;
        Assert.That(scene.SelectedBeatmap?.Version, Is.EqualTo("High"));
        Assert.That(
            droidStarsFrame
                .Elements.Single(element => element.Id == "songselect-diff-row-0-title")
                .Text,
            Does.Contain("High")
        );
        Assert.That(
            droidStarsFrame.Elements.Any(element => element.Id == "songselect-diff-row-1-title"),
            Is.False
        );
    }
    ...
    public void FolderFilterPopupUsesOsuDroidBottomAnchoredCollectionsPanel()
    {
        ...
        UiElementSnapshot panel = frame.Elements.Single(element =>
            element.Id == "songselect-collections-panel"
        );
        ...
        Assert.That(
            panel.Bounds.Bottom,
            Is.EqualTo(viewport.VirtualHeight - 20f * dp).Within(0.001f)
        );
        Assert.That(panel.CornerRadius, Is.EqualTo(14f * dp));
        Assert.That(panel.Color, Is.EqualTo(UiColor.Opaque(19, 19, 26)));
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-collections-new").Text,
            Is.EqualTo("Create new folder")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-collection-0-name").Text,
            Is.EqualTo("Default")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-collection-1-name").Text,
            Is.EqualTo("Folder")
        );
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-collection-0-delete"),
            Is.False
        );
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-collection-0-toggle"),
            Is.False
        );
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-collection-0-count"),
            Is.False
        );
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-collection-0-selected"),
            Is.True
        );
    }
    ...
    public void FolderFilterPopupShowsDefaultWhenNoCustomCollectionsExist()
    {
        ...
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-collection-1-name"),
            Is.False
        );
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-collections-empty"),
            Is.False
        );
    }
    ...
    public void SelectingDefaultFolderClearsFolderFilterAndRestoresOverlayLabel()
    {
        ...
        UiFrameSnapshot filteredFrame = scene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;
        Assert.That(
            filteredFrame
        ...
        UiFrameSnapshot defaultFrame = scene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;
        Assert.That(
            defaultFrame
