...
public sealed partial class SongSelectSceneTests
{
    ...
    public void SnapshotUsesCoverBackgroundAndStarColoredDifficultyButtons()
    {
        ...
        var scene = new SongSelectScene(
            new FakeLibrary(CreateSnapshot("bg.jpg")),
        ...
        Assert.That(
            frame.Elements.Any(element =>
                element.Id == "songselect-beatmap-background"
                && element.SpriteFit == UiSpriteFit.Cover
            ),
            Is.True
        );
        ...
        Assert.That(
            frame.Elements.Any(element =>
                element.Id == "songselect-diff-row-0-star-0"
                && element.AssetName == DroidAssets.SongSelectStar
            ),
            Is.True
        );
        UiElementSnapshot difficultyRow = frame.Elements.Single(element =>
            element.Id == "songselect-diff-row-0"
        );
        UiElementSnapshot fractionalStar = frame.Elements.Single(element =>
            element.Id == "songselect-diff-row-0-star-half"
        );
        ...
        Assert.That(
            fractionalStar.Bounds.X,
            Is.EqualTo(difficultyRow.Bounds.X + 60f + 52f * 2f + (46f - 46f * 0.4f) / 2f)
                .Within(0.01f)
        );
        Assert.That(
            fractionalStar.Bounds.Y,
            Is.EqualTo(difficultyRow.Bounds.Y + 50f + (47f - 47f * 0.4f) / 2f).Within(0.01f)
        );
        Assert.That(fractionalStar.SpriteSource, Is.Null);
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-detail-panel"),
            Is.False
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-top-overlay").Bounds.X,
            Is.EqualTo(-1640f)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-top-overlay").Alpha,
            Is.EqualTo(0.6f)
        );
        Assert.That(frame.Elements.Any(element => element.Id == "songselect-top-line"), Is.False);
        Assert.That(
            frame.Elements.Any(element =>
                element.Id == "songselect-back" && element.AssetName == DroidAssets.SongSelectBack
            ),
            Is.True
        );
    ...
    public void BottomControlsUseDroidSkinSprites()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-back").AssetName,
            Is.EqualTo(DroidAssets.SongSelectBack)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-mods").AssetName,
            Is.EqualTo(DroidAssets.SongSelectMods)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-options").AssetName,
            Is.EqualTo(DroidAssets.SongSelectOptions)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-random").AssetName,
            Is.EqualTo(DroidAssets.SongSelectRandom)
        );
        Assert.That(
            frame.Elements.Any(element => element.Text is "Mods" or "Beatmap Options" or "Random"),
            Is.False
        );
    }
    ...
    public void OnlineScorePanelIsHiddenUntilServerConnectionIsOn()
    {
        ...
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-scoring-switcher"),
            Is.True
        );
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-score-panel"),
            Is.False
        );
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-score-player"),
            Is.False
        );
    }
    ...
    public void OnlineScorePanelUsesOsuDroidBoundsWhenServerConnectionIsOn()
    {
        var scene = new SongSelectScene(
            ...
            new FakeDifficultyService(),
            CreateSongsRoot("audio.mp3"),
            OnlineProfilePanelState.Connecting
        );
        ...
        UiFrameSnapshot frame = scene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;
        UiFrameSnapshot mainMenuFrame = new MainMenuScene(
            onlinePanelState: OnlineProfilePanelState.Connecting
        )
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame;
        UiElementSnapshot mainMenuPanel = mainMenuFrame.Elements.Single(element =>
            element.Id == "profile-panel"
        );
        UiElementSnapshot mainMenuAvatarFooter = mainMenuFrame.Elements.Single(element =>
            element.Id == "profile-avatar-footer"
        );
        UiElementSnapshot scorePanel = frame.Elements.Single(element =>
            element.Id == "songselect-score-panel"
        );
        UiElementSnapshot scoreAvatarFooter = frame.Elements.Single(element =>
            element.Id == "songselect-score-avatar-footer"
        );
    ...
    {
        var state = new OnlineProfilePanelState(
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-score-player").Text,
            Is.EqualTo("Player")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-score-rank").Text,
            Is.EqualTo("#42")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-score-pp").Text,
            Is.EqualTo("Performance: 12,345pp")
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-score-acc").Text,
            Is.EqualTo("Accuracy: 98.76%")
        );
    }
    ...
    public void ScoringSwitcherStaysDisabledUntilOnlineScoringIsEnabled()
    {
        ...
        var snapshot = new BeatmapLibrarySnapshot([
            new BeatmapSetInfo(1, "1 Artist - Title", [rankedBeatmap]),
        ]);
        var scene = new SongSelectScene(
            new FakeLibrary(snapshot),
            new NoOpMenuMusicController(),
            new FakeDifficultyService(),
            CreateSongsRoot("audio.mp3")
        );
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "songselect-scoring-switcher").AssetName,
            Is.EqualTo(DroidAssets.RankingDisabled)
        );
    }
    ...
    public void SelectedSetUsesOsuDroidExpandedRowSpacingAndCentering()
    {
        ...
        var secondSet = new BeatmapSetInfo(
            2,
            "2 Second",
            [
        ...
        var thirdSet = new BeatmapSetInfo(
            3,
            "3 Third",
            [CreateBeatmap("Easy", null, 1.5f, setId: 3, setDirectory: "3 Third")]
        );
        var scene = new SongSelectScene(
            new FakeLibrary(new BeatmapLibrarySnapshot([firstSet, secondSet, thirdSet])),
            new NoOpMenuMusicController(),
            new FakeDifficultyService(),
            CreateSongsRoot("audio.mp3")
        );
        ...
        UiRect[] rows = Enumerable
            .Range(0, 4)
            .Select(index =>
                frame
                    .Elements.Single(element => element.Id == $"songselect-diff-row-{index}")
                    .Bounds
            )
            .ToArray();
        ...
        UiRect followingSet = frame
            .Elements.Single(element => element.Id == "songselect-set-1")
            .Bounds;
        Assert.That(followingSet.Y, Is.EqualTo(564f).Within(0.01f));
        Assert.That(
            followingSet.Y - (rows[^1].Y + rows[^1].Height),
    ...
    public void SelectingSetAnimatesDifficultyRowsFromCollapsedToExpanded()
    {
        ...
        var scene = new SongSelectScene(
            new FakeLibrary(new BeatmapLibrarySnapshot([firstSet, secondSet])),
            new NoOpMenuMusicController(),
            new FakeDifficultyService(),
            CreateSongsRoot("audio.mp3")
        );
        ...
        scene.Enter();
        scene.SelectSet(1);
        float[] collapsed = Enumerable
            .Range(0, 3)
            .Select(index =>
                scene
                    .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
                    .UiFrame.Elements.Single(element =>
                        element.Id == $"songselect-diff-row-{index}"
                    )
                    .Bounds.Y
            )
            .ToArray();
        scene.Update(TimeSpan.FromSeconds(0.25));
        float[] expanding = Enumerable
            .Range(0, 3)
            .Select(index =>
                scene
                    .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
    ...
    {
        BeatmapSetInfo[] sets = Enumerable
            .Range(0, 9)
            .Select(index => new BeatmapSetInfo(
                index + 1,
                $"{index + 1} Set",
                [
                    CreateBeatmap(
                        ...
                        1.5f,
                        setId: index + 1,
                        setDirectory: $"{index + 1} Set"
                    ),
                ]
            ))
            .ToArray();
        var scene = new SongSelectScene(
            new FakeLibrary(new BeatmapLibrarySnapshot(sets)),
            new NoOpMenuMusicController(),
            new FakeDifficultyService(),
            CreateSongsRoot("audio.mp3")
        );
        ...
        UiRect[] rows = scene
            ...
            .UiFrame.Elements.Where(element =>
                element.Id.StartsWith("songselect-set-", StringComparison.Ordinal)
                && element.Kind == UiElementKind.Sprite
            )
        ...
        UiRect centerRow = rows.MinBy(row =>
            MathF.Abs(row.Y + row.Height * 0.5f - viewport.VirtualHeight * 0.5f)
        );
    ...
    public void SongSelectDragContinuesAfterRelease()
    {
        ...
        float firstRowY = afterDrag
            .Elements.Single(element => element.Id == "songselect-set-0")
            .Bounds.Y;
        ...
        Assert.That(
            afterInertia.Elements.Single(element => element.Id == "songselect-set-0").Bounds.Y,
            Is.LessThan(firstRowY)
        );
    }
    ...
    public void DifficultyBackgroundSwitchesImmediatelyLikeOsuDroidSongMenu()
    {
        ...
        Assert.That(
            frame.Elements.Any(element => element.Id == "songselect-previous-beatmap-background"),
            Is.False
        );
        ...
        UiElementSnapshot fadedBackground = scene
            .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
            .UiFrame.Elements.Single(element => element.Id == "songselect-beatmap-background");
