...
public sealed partial class SongSelectSceneTests
{
    private sealed class FakeLibrary(BeatmapLibrarySnapshot snapshot) : IBeatmapLibrary
    {
        ...
        private readonly Dictionary<string, HashSet<string>> _collections = new(
        ...
        public BeatmapOptions GetOptions(string setDirectory) =>
            _options.TryGetValue(setDirectory, out BeatmapOptions? value)
                ? value
                : new BeatmapOptions(setDirectory);

        public void SaveOptions(BeatmapOptions nextOptions) =>
            _options[nextOptions.SetDirectory] = nextOptions;
        ...
        public IReadOnlyList<BeatmapCollection> GetCollections(
            ...
            _collections
                ...
                .Select(pair => new BeatmapCollection(
                    pair.Key,
                    pair.Value.Count,
                    selectedSetDirectory is not null && pair.Value.Contains(selectedSetDirectory)
                ))
                .ToArray();

        public IReadOnlySet<string> GetCollectionSetDirectories(string name) =>
            _collections.TryGetValue(name, out HashSet<string>? sets)
        ...
        {
            _snapshot = new BeatmapLibrarySnapshot(
                _snapshot
                    .Sets.Where(set =>
                        !string.Equals(set.Directory, directory, StringComparison.Ordinal)
                    )
                    .ToArray()
            );
