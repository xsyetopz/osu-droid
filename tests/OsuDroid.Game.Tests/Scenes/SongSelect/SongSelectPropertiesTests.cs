...
public sealed partial class SongSelectSceneTests
{
    ...
    public void PropertiesPopupUsesMaterialIconsInsteadOfTextGlyphs()
    {
        ...
        Assert.That(
            frame
                .Elements.Single(element => element.Id == "songselect-properties-favorite-icon")
                .MaterialIcon,
            Is.EqualTo(UiMaterialIcon.HeartOutline)
        );
        Assert.That(
            frame
                .Elements.Single(element => element.Id == "songselect-properties-manage-icon")
                .MaterialIcon,
            Is.EqualTo(UiMaterialIcon.Folder)
        );
        Assert.That(
            frame
                .Elements.Single(element => element.Id == "songselect-properties-delete-icon")
                .MaterialIcon,
            Is.EqualTo(UiMaterialIcon.Delete)
        );
        Assert.That(
            frame
                .Elements.Where(element => element.Text is not null)
                .Select(element => element.Text),
            Has.None.Contains("☐ Add to Favorites")
        );
        Assert.That(
            frame
                .Elements.Where(element => element.Text is not null)
                .Select(element => element.Text),
            Has.None.Contains("▣ Manage Favorites")
        );
        Assert.That(
            frame
                .Elements.Where(element => element.Text is not null)
                .Select(element => element.Text),
            Has.None.Contains("⌫ Delete beatmap")
        );
    }
    ...
    public void CollectionsCanBeCreatedToggledAndDeleted()
    {
        ...
        Assert.That(
            library.GetCollections("1 Artist - Title").Single().ContainsSelectedSet,
            Is.True
        );
    ...
    public void DeleteBeatmapRemovesSelectedSet()
    {
        ...
        Assert.That(
            scene
                .CreateSnapshot(VirtualViewport.FromSurface(1280, 720))
                .UiFrame.Elements.Any(element => element.Id == "songselect-properties-panel"),
            Is.False
        );
    }
}
