...
public sealed partial class UiCompatibilityTests
{
    ...
    public void ShownMenuAutoCollapsesAfterAndroidIdleTimeout()
    {
        ...
        Assert.That(
            scene
                ...
                .UiFrame.Elements.Any(element => element.Id == "menu-0"),
            Is.False
        );
    }
    ...
    public void MainMenuCollapseUsesAndroidFadeAndLeftDrift()
    {
        var scene = new MainMenuScene();
        var viewport = VirtualViewport.FromSurface(1280, 720);
        UiElementSnapshot expanded = ExpandedFrame(scene, viewport)
            .Elements.Single(element => element.Id == "menu-0");

        scene.ToggleCookie();
        scene.Update(
            TimeSpan.FromMilliseconds(MainMenuScene.MenuCollapseDurationMilliseconds / 2d)
        );
        UiElementSnapshot collapsing = scene
    ...
    public void MainMenuPressTintMatchesAndroidButtonColor()
    {
        ...
        scene.ReleasePress();
        UiElementSnapshot released = scene
            .CreateSnapshot(viewport)
            .UiFrame.Elements.Single(element => element.Id == "menu-0");
    ...
    public void MainMenuDownloaderTabUsesAndroidPressTint()
    {
        ...
        UiElementSnapshot tab = scene
            .CreateSnapshot(viewport)
            .UiFrame.Elements.Single(element => element.Id == "beatmap-downloader");
    ...
    public void MainMenuExitUsesAndroidFadeOutBeforeRoute()
    {
        ...
        var viewport = VirtualViewport.FromSurface(1280, 720);
        UiElementSnapshot expanded = ExpandedFrame(scene, viewport)
            .Elements.Single(element => element.Id == "logo");
        ...
        Assert.That(
            prompt.Elements.Any(element =>
                element.Id == "exit-dialog-title" && element.Text == "Exit"
            ),
            Is.True
        );
        Assert.That(
            prompt.Elements.Any(element =>
                element.Id == "exit-dialog-message"
                && element.Text == "Are you sure you want to exit the game?"
            ),
            Is.True
        );
        Assert.That(
            prompt.Elements.Single(element => element.Id == "exit-dialog-confirm").Action,
            Is.EqualTo(UiAction.MainMenuExitConfirm)
        );
        Assert.That(
            prompt.Elements.Single(element => element.Id == "exit-dialog-cancel").Action,
            Is.EqualTo(UiAction.MainMenuExitCancel)
        );
        ...
        UiElementSnapshot blackout = midway.Elements.Single(element =>
            element.Id == "exit-blackout"
        );
        ...
        Assert.That(
            completed.Elements.Any(element =>
                element.Id == "exit-instruction"
                && element.Text == "Done playing? Swipe this app away to close it."
            ),
            Is.True
        );
