...
public sealed partial class OptionsSceneTests
{
    ...
    public void OptionsSceneSliderUsesAndroidSeekbarLayoutAndDrawableMetrics()
    {
        ...
        UiElementSnapshot sliderRow = frame.Elements.Single(element =>
            element.Id == "options-row-0"
        );
        UiElementSnapshot label = frame.Elements.Single(element =>
        ...
        UiElementSnapshot value = frame.Elements.Single(element =>
            element.Id == "options-row-0-value"
        );
        UiElementSnapshot track = frame.Elements.Single(element =>
            element.Id == "options-row-0-slider-track"
        );
        UiElementSnapshot fill = frame.Elements.Single(element =>
            element.Id == "options-row-0-slider-fill"
        );
        UiElementSnapshot thumb = frame.Elements.Single(element =>
            element.Id == "options-row-0-slider-thumb"
        );

        Assert.That(
            label.Bounds.X,
            Is.EqualTo(sliderRow.Bounds.X + DroidUiMetrics.SeekbarContainerMarginX).Within(0.001f)
        );
        Assert.That(summary.Bounds.Right, Is.LessThan(value.Bounds.X));
        Assert.That(summary.Bounds.Bottom, Is.LessThan(track.Bounds.Y));
        Assert.That(
            value.Bounds.Right,
            Is.EqualTo(sliderRow.Bounds.Right - DroidUiMetrics.SeekbarContainerMarginX)
                .Within(0.001f)
        );
        Assert.That(value.TextStyle!.Alignment, Is.EqualTo(UiTextAlignment.Right));
        Assert.That(
            track.Bounds.X,
            Is.EqualTo(sliderRow.Bounds.X + DroidUiMetrics.SeekbarTrackMarginX).Within(0.001f)
        );
        Assert.That(
            track.Bounds.Right,
            Is.EqualTo(sliderRow.Bounds.Right - DroidUiMetrics.SeekbarTrackMarginX).Within(0.001f)
        );
        Assert.That(
            track.Bounds.Height,
            Is.EqualTo(DroidUiMetrics.SeekbarTrackHeight).Within(0.001f)
        );
        ...
        Assert.That(
            ...
            Is.EqualTo(track.Bounds.Right).Within(0.001f)
        );
        Assert.That(thumb.Bounds.Width, Is.EqualTo(DroidUiMetrics.SeekbarThumbSize).Within(0.001f));
        Assert.That(
            thumb.Bounds.Height,
            Is.EqualTo(DroidUiMetrics.SeekbarThumbSize).Within(0.001f)
        );
    ...
    public void OptionsSceneLongSliderSummaryWrapsWithoutShrinkingText()
    {
        ...
        UiElementSnapshot title = frame.Elements.Single(element =>
            element.Id == "options-row-3-label"
        );
        UiElementSnapshot firstSummaryLine = frame.Elements.Single(element =>
            element.Id == "options-row-3-summary"
        );
        UiElementSnapshot secondSummaryLine = frame.Elements.Single(element =>
            element.Id == "options-row-3-summary-1"
        );
        UiElementSnapshot finalSummaryLine = frame.Elements.Single(element =>
            element.Id == "options-row-3-summary-3"
        );
        UiElementSnapshot track = frame.Elements.Single(element =>
            element.Id == "options-row-3-slider-track"
        );

        Assert.That(
            row.Bounds.Height,
            Is.EqualTo(DroidUiMetrics.LongSliderRowHeight).Within(0.001f)
        );
        ...
        Assert.That(
            firstSummaryLine.TextStyle!.Size,
        ...
        Assert.That(
            track.Bounds.X,
            Is.EqualTo(row.Bounds.X + DroidUiMetrics.SeekbarTrackMarginX).Within(0.001f)
        );
    }
    ...
    {
        string corePath =
            "/var/mobile/Containers/Data/Application/79AEE6E3-E6B9-47A2-980F-055F26C2F8B7/Library/osu!droid";
        string hyphenatedCorePath = corePath.Replace(
            "/Library/osu!droid",
            "/Library/osu-droid",
        ...
        var defaults = new OptionsPathDefaults(
            ...
            $"{corePath}/Skin",
            $"{corePath}/Songs",
            UsesNativeDefaultSummaries: true
        );
        var settings = new MemorySettingsStore();
        ...
        string?[] values = frame
            .Elements.Where(element =>
                element.Id.EndsWith("-input-value", StringComparison.Ordinal)
            )
            .Select(element => element.Text)
            .ToArray();
        ...
        Assert.That(
            settings.GetString("skinTopPath", string.Empty),
            Is.EqualTo(defaults.SkinTopPath)
        );
        Assert.That(
            settings.GetString("directory", string.Empty),
            Is.EqualTo(defaults.SongsDirectory)
        );

        string?[] summaries = frame
            .Elements.Where(element => element.Id.EndsWith("-summary", StringComparison.Ordinal))
            .Select(element => element.Text)
            .ToArray();
        ...
        Assert.That(
            summaries,
            Does.Contain(
                "Path to directory containing skin files. (default: /var/mobile/…/Library/osu!droid/Skin)"
            )
        );
        Assert.That(
            summaries,
            Does.Contain(
                "Path to directory containing beatmaps (default: /var/mobile/…/Library/osu!droid/Songs)"
            )
        );
        Assert.That(
            summaries,
    ...
    {
        string path =
            "/very/long/custom/location/with/many/segments/that/does/not/match/ios/container/osu!droid/Songs";
        var settings = new MemorySettingsStore();
        settings.SetString("directory", path);
        var scene = new OptionsScene(
            new GameLocalizer(),
            settings,
            pathDefaults: new OptionsPathDefaults("/core", "/core/Skin", "/core/Songs")
        );
        var viewport = VirtualViewport.FromSurface(1280, 720);
        ...
        Assert.That(
            values.Any(value =>
                value is not null
                && value.Contains('…')
                && value.EndsWith("/container/osu!droid/Songs", StringComparison.Ordinal)
            ),
            Is.True
        );
    ...
    public void OptionsSceneSliderDragUpdatesValueAndSuppressesScroll()
    {
        ...
        var dragPoint = new UiPoint(
            track.Bounds.X + track.Bounds.Width * 0.75f,
            track.Bounds.Y + track.Bounds.Height / 2f
        );

        Assert.That(
            scene.TryBeginSliderDrag("options-row-0-slider-thumb", dragPoint, viewport),
            Is.True
        );
    ...
    public void OptionsSceneUsesAndroidHalfRoundedCategoryAndRows()
    {
        ...
        UiElementSnapshot category = frame.Elements.Single(element =>
            element.Id == "options-category-0-header"
        );
        UiElementSnapshot middleRow = frame.Elements.Single(element =>
            element.Id == "options-row-0"
        );
        UiElementSnapshot bottomRow = frame.Elements.Single(element =>
            element.Id == "options-row-2"
        );
        ...
        Assert.That(
            frame.Elements.Any(element => element.Id == "options-category-0-card"),
            Is.False
        );
    }
    ...
    public void OptionsSceneSidebarUnselectedTabsDoNotDrawVisibleBackgrounds()
    {
        ...
        UiElementSnapshot selected = frame.Elements.Single(element =>
            element.Id == "options-section-selected"
        );
        UiElementSnapshot unselectedHit = frame.Elements.Single(element =>
            element.Id == "options-section-1-hit"
        );
