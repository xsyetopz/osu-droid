...
public sealed partial class OptionsSceneTests
{
    ...
    public void OptionsSceneSelectsSidebarSectionsAndResetsContentScroll()
    {
        ...
        Assert.That(
            frame.Elements.Single(element => element.Id == "options-section-selected").Action,
            Is.EqualTo(UiAction.OptionsSectionAudio)
        );
        Assert.That(
            frame.Elements.Single(element => element.Id == "options-section-3-text").Text,
            Is.EqualTo("Audio")
        );
    ...
    public void OptionsSceneContentScrollMovesRowsAndKeepsSectionsFixed()
    {
        ...
        UiElementSnapshot initialRow = initial.Elements.Single(element =>
            element.Id == "options-row-0"
        );
        UiElementSnapshot initialSection = initial.Elements.Single(element =>
        ...
        UiElementSnapshot? scrolledRow = scrolled.Elements.SingleOrDefault(element =>
            element.Id == "options-row-0"
        );
        UiElementSnapshot scrolledSection = scrolled.Elements.Single(element =>
            element.Id == "options-section-selected"
        );
        ...
        Assert.That(
            scrolledRow?.Bounds.Y ?? float.NegativeInfinity,
            Is.LessThan(initialRow.Bounds.Y)
        );
    ...
    public void OptionsSceneSectionScrollMovesSidebarAndKeepsRowsFixed()
    {
        var scene = new OptionsScene(new GameLocalizer());
        var viewport = VirtualViewport.FromSurface(1280, 720);
        var sectionPoint = new UiPoint(
            DroidUiMetrics.ContentPaddingX + 10f,
            DroidUiMetrics.ContentTop
        );
        UiFrameSnapshot initial = scene.CreateSnapshot(viewport).UiFrame;
        UiElementSnapshot initialRow = initial.Elements.Single(element =>
        ...
        UiElementSnapshot initialSectionIcon = initial.Elements.Single(element =>
            element.Id == "options-section-0-icon"
        );

        scene.Scroll(160f, sectionPoint, viewport);
        UiFrameSnapshot scrolled = scene.CreateSnapshot(viewport).UiFrame;
        UiElementSnapshot scrolledRow = scrolled.Elements.Single(element =>
            element.Id == "options-row-0"
        );
        UiElementSnapshot? scrolledSectionIcon = scrolled.Elements.SingleOrDefault(element =>
            element.Id == "options-section-0-icon"
        );
        ...
        Assert.That(scene.ContentScrollOffset, Is.Zero);
        Assert.That(
            scrolledSectionIcon?.Bounds.Y ?? float.NegativeInfinity,
            Is.LessThan(initialSectionIcon.Bounds.Y)
        );
    ...
    public void OptionsSceneScrollClampsToAvailableContent()
    {
        ...
        Assert.That(
            scene.ContentScrollOffset,
            Is.EqualTo(OptionsScene.MaxContentScrollOffset(viewport))
        );
        ...
        Assert.That(
            scene.SectionScrollOffset,
            Is.EqualTo(OptionsScene.MaxSectionScrollOffset(viewport))
        );
