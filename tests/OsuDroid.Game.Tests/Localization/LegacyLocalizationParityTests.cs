...
public sealed class LegacyLocalizationParityTests
{
    ...
    [Test]
    public void LegacyAndroidResourceStringsAreAvailableInEnglishCatalog()
    {
        var localizer = new GameLocalizer();
        (string Name, string Value)[] legacyStrings = LoadLegacyAndroidStrings();

        foreach ((string? name, string? value) in legacyStrings)
        {
            Assert.That(localizer[$"Legacy_{name}"], Is.EqualTo(value), name);
        }
    }

    [Test]
    public void LegacyLanguagePackEnglishStringsAreAvailableInEnglishCatalog()
    {
        var localizer = new GameLocalizer();
        ...
        {
            Assert.That(localizer[$"LegacyLanguagePack_{name}"], Is.EqualTo(value), name);
        }
    }

    [Test]
    public void LegacyStringReferencesResolveToLocalOrLanguagePackResources()
    {
        var knownNames = LoadLegacyAndroidStrings().Select(item => item.Name)
            .Concat(LoadLanguagePackEnglishStrings().Select(item => item.Name))
            .Concat(["ok", "cancel"])
            .ToHashSet(StringComparer.Ordinal);
        (string Name, string Path, int Line)[] references = LoadLegacyStringReferences();
    ...
    [Test]
    public void EnglishCatalogIsGeneratedFromLegacySources()
    {
        DirectoryInfo root = RepoRoot();
        string script = Path.Combine(root.FullName, "scripts", "dev", "generate-legacy-localization.py");
    ...
    [Test]
    public void CriticalLegacyUiStringsMatchLatestUpstreamEnglish()
    {
        var localizer = new GameLocalizer();
    ...
    private static (string Name, string Value)[] LoadLegacyAndroidStrings()
    {
    ...
    private static (string Name, string Path, int Line)[] LoadLegacyStringReferences()
    {
        DirectoryInfo root = RepoRoot();
        string legacyRoot = Path.Combine(root.FullName, "third_party", "osu-droid-legacy");
        ...
        foreach (string? path in Directory.EnumerateFiles(legacyRoot, "*.*", SearchOption.AllDirectories)
